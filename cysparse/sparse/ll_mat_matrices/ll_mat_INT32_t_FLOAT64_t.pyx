...
from cysparse.sparse.ll_mat_views.ll_mat_view_INT32_t_FLOAT64_t cimport PyLLSparseMatrixView_Check
...
from cysparse.sparse.csc_mat_matrices.csc_mat_INT32_t_FLOAT64_t cimport MakeCSCSparseMatrix_INT32_t_FLOAT64_t
...
include "ll_mat_kernel/ll_mat_assignment_kernel_INT32_t_FLOAT64_t.pxi"
include "ll_mat_kernel/ll_mat_multiplication_by_numpy_vector_kernel_INT32_t_FLOAT64_t.pxi"
include "ll_mat_helpers/ll_mat_multiplication_INT32_t_FLOAT64_t.pxi"
include "ll_mat_helpers/ll_mat_addition_INT32_t_FLOAT64_t.pxi"
include "ll_mat_helpers/ll_mat_is_symmetric_INT32_t_FLOAT64_t.pxi"
...
########################################################################################################################
from cysparse.sparse.ll_mat_views.ll_mat_view_INT32_t_FLOAT64_t cimport LLSparseMatrixView_INT32_t_FLOAT64_t, MakeLLSparseMatrixView_INT32_t_FLOAT64_t
...
cdef class LLSparseMatrix_INT32_t_FLOAT64_t(MutableSparseMatrix_INT32_t_FLOAT64_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT32_t_FLOAT64_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def create_transpose(self):
        ...
        if self.__store_symmetric:
            ...
            transpose = LLSparseMatrix_INT32_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT32_t_FLOAT64_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind, col=col,
                                                     val=val,
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT32_t_FLOAT64_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind,
                                                     row=row,
    ...
    ####################################################################################################################
    #                                            ### ASSIGN ###
    cdef assign(self, LLSparseMatrixView_INT32_t_FLOAT64_t view, object obj):
        """
        Set ``A[..., ...] = obj`` directly.

        Args:
            view: An ``LLSparseMatrixView_INT32_t_FLOAT64_t`` that points to this matrix (``self``).
            obj: Can be a:
                - ``LLSparseMatrix``;
                - ``LLSparseMatrixView``;
        ...
        cdef:
            ...
            LLSparseMatrixView_INT32_t_FLOAT64_t A_view
        ...
        if self.__store_symmetric:
            if PySparseMatrix_Check(obj):
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A = <LLSparseMatrix_INT32_t_FLOAT64_t> obj
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A_view = <LLSparseMatrixView_INT32_t_FLOAT64_t> obj
    ...
    def __setitem__(self, tuple key, value):
        ...
        cdef LLSparseMatrixView_INT32_t_FLOAT64_t view
        ...
        if not PyInt_Check(<PyObject *>key[0]) or not PyInt_Check(<PyObject *>key[1]):
            ...
            view = MakeLLSparseMatrixView_INT32_t_FLOAT64_t(self, <PyObject *>key[0], <PyObject *>key[1])
    ...
    def tril(self, int k = 0):
        ...
        cdef:
            INT32_t i, j, k_
            LLSparseMatrix_INT32_t_FLOAT64_t ll_mat_tril

        ll_mat_tril = LLSparseMatrix_INT32_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    def triu(self, int k = 0):
        ...
        cdef:
            INT32_t i, j, k_
            LLSparseMatrix_INT32_t_FLOAT64_t ll_mat_triu

        ll_mat_triu = LLSparseMatrix_INT32_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    # Addition
    ####################################################################################################################
    def shift(self, sigma, LLSparseMatrix_INT32_t_FLOAT64_t B):
        ...
        if self.__store_symmetric == B.__store_symmetric:
            ...
            for i from 0 <= i < B.__nrow:
                ...
                while k != -1:
                    ...
                    update_ll_mat_item_add_INT32_t_FLOAT64_t(self, i, j, v)
                    if i != j:
                        update_ll_mat_item_add_INT32_t_FLOAT64_t(self, j, i, v)
                    k = B.link[k]
        else:
    ...
    def update_add_at(self, id1, id2, val):
        ...
        See :meth:`update_add_at_with_numpy_arrays_INT32_t_FLOAT64_t`.
        """
        return update_add_at_with_numpy_arrays_INT32_t_FLOAT64_t(self, id1, id2, val)
    ...
    def matvec(self, b):
        ...
        return multiply_ll_mat_with_numpy_vector_INT32_t_FLOAT64_t(self, b)
...
# Factory methods
########################################################################################################################
cdef MakeLLSparseMatrix_INT32_t_FLOAT64_t(INT32_t nrow,
                                        INT32_t ncol,
                                        INT32_t nnz,
                                        INT32_t free,
                                        INT32_t nalloc,
    ...
    """
    cdef LLSparseMatrix_INT32_t_FLOAT64_t ll_mat

    ll_mat = LLSparseMatrix_INT32_t_FLOAT64_t(control_object=unexposed_value,
                                           no_memory=True,
                                           size_hint=nalloc,
                                           nrow=nrow,
                                           ncol=ncol,
