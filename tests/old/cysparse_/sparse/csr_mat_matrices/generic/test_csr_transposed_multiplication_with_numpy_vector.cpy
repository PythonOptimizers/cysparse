...
{% for index_type in index_list %}
  ...
  {% for element_type in type_list %}
        ...
        csr_y = self.l_@outerloop.index@_@loop.index@_csr.matvec_transp(self.x_@element_type@)
...
{% for index_type in index_list %}
  ...
  {% for element_type in type_list %}
        ...
        self.l_@outerloop.index@_@loop.index@ = NewLinearFillLLSparseMatrix(nrow=self.nrow, ncol=self.ncol, itype=@index_type|type2enum@, dtype=@element_type|type2enum@, row_wise=True)
...
{% for index_type in index_list %}
  ...
  {% for element_type in type_list %}
        l_y = self.l_@outerloop.index@_@loop.index@.matvec_transp(self.x_@element_type@)
        csr_y = self.l_@outerloop.index@_@loop.index@_csr.matvec_transp(self.x_strided_@element_type@[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  {% endfor %}
{% endfor %}
