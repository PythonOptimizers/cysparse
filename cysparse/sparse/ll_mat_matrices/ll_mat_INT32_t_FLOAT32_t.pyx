...
cdef class LLSparseMatrix_INT32_t_FLOAT32_t(MutableSparseMatrix_INT32_t_FLOAT32_t):
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT32_t_FLOAT32_t(nrow=self.__nrow, ncol=self.__ncol, nnz=self.__nnz, ind=ind, col=col, val=val, is_symmetric=self.__is_symmetric, store_zeros=self.__store_zeros)
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT32_t_FLOAT32_t(nrow=self.__nrow, ncol=self.__ncol, nnz=self.__nnz, ind=ind, row=row, val=val, is_symmetric=self.__is_symmetric, store_zeros=self.__store_zeros)
    ...
    cdef assign(self, LLSparseMatrixView_INT32_t_FLOAT32_t view, object obj):
        ...
        if self.__is_symmetric:
            if PySparseMatrix_Check(obj):
                for i from 0 <= i < nrow:
                    for j from 0 <= j <= i:
                        self.put(row_indices[i], col_indices[j], <FLOAT32_t> obj.at(i, j))
