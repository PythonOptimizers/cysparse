...
cdef cnp.ndarray[cnp.npy_int32, ndim=1, mode='c'] multiply_csc_mat_with_numpy_vector_INT64_t_INT32_t(CSCSparseMatrix_INT64_t_INT32_t A, cnp.ndarray[cnp.npy_int32, ndim=1] b):
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.is_symmetric:
            pass
            #multiply_sym_csc_mat_with_numpy_vector_kernel_INT64_t_INT32_t(A_nrow, b_data, c_data, A.val, A.row, A.ind)
        else:
            multiply_csc_mat_with_numpy_vector_kernel_INT64_t_INT32_t(A_nrow, A_ncol, b_data, c_data, A.val, A.row, A.ind)
    else:
        pass
        #if A.is_symmetric:
        #    multiply_sym_csc_mat_with_strided_numpy_vector_kernel_INT64_t_INT32_t(A.nrow,
        #                                                         b_data, b.strides[0] / sd,
        #                                                         c_data, c.strides[0] / sd,
        #                                                         A.val, A.row, A.ind)
        #else:
        #    multiply_csc_mat_with_strided_numpy_vector_kernel_INT64_t_INT32_t(A.nrow,
        #                                                     b_data, b.strides[0] / sd,
        #                                                     c_data, c.strides[0] / sd,
        #                                                     A.val, A.row, A.ind)
