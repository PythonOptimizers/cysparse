...
##################################################################
cdef CSCSparseMatrix_INT32_t_FLOAT64_t cholmod_sparse_to_CSCSparseMatrix_INT32_t_FLOAT64_t(cholmod_sparse * sparse_struct, bint no_copy=False):
    """
    Convert a ``cholmod`` sparse struct to a :class:`CSCSparseMatrix_INT32_t_FLOAT64_t`.
    ...
    csc_mat = MakeCSCSparseMatrix_INT32_t_FLOAT64_t(nrow=nrow, ncol=ncol, nnz=nnz, ind=ind, row=row, val=val, is_symmetric=is_symmetric, store_zeros=False)
