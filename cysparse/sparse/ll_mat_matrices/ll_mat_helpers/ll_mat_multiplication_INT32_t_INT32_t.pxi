...
######################
cdef cnp.ndarray[cnp.npy_int32, ndim=1, mode='c'] multiply_ll_mat_with_numpy_vector2_INT32_t_INT32_t(LLSparseMatrix_INT32_t_INT32_t A, cnp.ndarray[cnp.npy_int32, ndim=1, mode='c'] b):
    """
    Multiply a :class:`LLSparseMatrix` ``A`` with a numpy vector ``b``.
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__is_symmetric:
            ...
            multiply_ll_mat_with_numpy_vector_kernel2_INT32_t_INT32_t(A_nrow, b_data, c_data, A.val, A.col, A.link, A.root)
    else:
        pass
        #if A.__is_symmetric:
        #    multiply_sym_ll_mat_with_strided_numpy_vector_kernel_INT32_t_INT32_t(A.nrow,
