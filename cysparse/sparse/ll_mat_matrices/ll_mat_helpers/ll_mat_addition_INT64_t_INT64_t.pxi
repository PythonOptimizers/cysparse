...
cdef update_add_at_with_numpy_arrays_INT64_t_INT64_t(LLSparseMatrix_INT64_t_INT64_t A,
    ...
    if cnp.PyArray_ISCONTIGUOUS(val):
        for i from 0<= i < array_size:
            update_ll_mat_item_add_INT64_t_INT64_t(A, id1_data[i], id2_data[i], val_data[i])
    else:
        # val is not C-contiguous
        for i from 0<= i < array_size:
            update_ll_mat_item_add_INT64_t_INT64_t(A, id1_data[i], id2_data[i], val_data[i * incx])
