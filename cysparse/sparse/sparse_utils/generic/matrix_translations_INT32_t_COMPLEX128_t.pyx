...
cdef csc_to_csr_kernel_INT32_t_COMPLEX128_t(INT32_t nrow, INT32_t ncol, INT32_t nnz,
                                      INT32_t * csc_ind, INT32_t * csc_row, COMPLEX128_t * csc_val,
                                      INT32_t * csr_ind, INT32_t * csr_col, COMPLEX128_t * csr_val):
    """
    Translate an CSR to an CSC matrix format.
...
cdef csr_to_ll_kernel_INT32_t_COMPLEX128_t(INT32_t nrow, INT32_t ncol, INT32_t nnz,
                                      INT32_t * csr_ind, INT32_t * csr_col, COMPLEX128_t * csr_val,
                                      INT32_t * ll_root, INT32_t * ll_col, INT32_t * ll_link, COMPLEX128_t * ll_val):
    """
    ...
    memcpy(ll_val, csr_val, nnz * sizeof(COMPLEX128_t))
...
cdef csc_to_ll_kernel_INT32_t_COMPLEX128_t(INT32_t nrow, INT32_t ncol, INT32_t nnz,
                                      INT32_t * csc_ind, INT32_t * csc_row, COMPLEX128_t * csc_val,
                                      INT32_t * ll_root, INT32_t * ll_col, INT32_t * ll_link, COMPLEX128_t * ll_val):
    """
    Translate an CSR to an LL matrix format.
    ...
    cdef COMPLEX128_t * val = <COMPLEX128_t *> PyMem_Malloc(nnz * sizeof(COMPLEX128_t))
    if not val:
        PyMem_Free(ind)
        PyMem_Free(col)
        raise MemoryError()

    csc_to_csr_kernel_INT32_t_COMPLEX128_t(nrow, ncol, nnz, csc_ind, csc_row, csc_val, ind, col, val)

    ############
    # then transform CSR to LL
    ############
    csr_to_ll_kernel_INT32_t_COMPLEX128_t(nrow, ncol, nnz,
                                    ind, col, val,
