...
# MUMPS CONTEXT
################################################################
cdef class MumpsContext_INT32_t_COMPLEX128_t:
    """
    Mumps Context.
    ...
    def __cinit__(self, LLSparseMatrix_INT32_t_COMPLEX128_t A, comm_fortran=-987654, verbose=False):
        """
        Args:
        ...
        self.a_val = <COMPLEX128_t *> PyMem_Malloc(self.nnz * sizeof(COMPLEX128_t))
    ...
    #property cntl:
    #    def __get__(self):
    #        cdef ZMUMPS_REAL[::1] view = <ZMUMPS_REAL[::1]> self.params.cntl
    #        return view
    ...
    # Solve
    ####################################################################################################################
    cdef solve_dense(self, COMPLEX128_t * rhs, INT32_t rhs_length, INT32_t nrhs):
        """
    ...
    cdef solve_sparse(self, INT32_t * rhs_col_ptr, INT32_t * rhs_row_ind,
                       COMPLEX128_t * rhs_val, INT32_t rhs_nnz, INT32_t nrhs, COMPLEX128_t * x, INT32_t x_length):
        """
    ...
    def solve(self, **kwargs):
        ...
        if 'rhs' in kwargs:
            ...
            assert are_mixed_types_compatible(COMPLEX128_T, rhs.dtype), "Rhs matrix or vector must have a Numpy compatible type (%s)!" % cysparse_to_numpy_type(COMPLEX128_T)
            ...
            self.solve_dense(<COMPLEX128_t *> cnp.PyArray_DATA(x), rhs_shape[0], nrhs)
        ...
        ################################################################################################################
            ...
            self.solve_sparse(<INT32_t *>cnp.PyArray_DATA(rhs_col_ptr), <INT32_t *>cnp.PyArray_DATA(rhs_row_ind), <COMPLEX128_t *> cnp.PyArray_DATA(rhs_val),
                        rhs_nnz, nrhs, <COMPLEX128_t *> cnp.PyArray_DATA(x), x_length)
        else:
