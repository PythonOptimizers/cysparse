...
from cysparse.sparse.ll_mat_matrices.ll_mat_INT32_t_INT64_t cimport LLSparseMatrix_INT32_t_INT64_t, MakeLLSparseMatrix_INT32_t_INT64_t
from cysparse.sparse.csc_mat_matrices.csc_mat_INT32_t_INT64_t cimport CSCSparseMatrix_INT32_t_INT64_t, MakeCSCSparseMatrix_INT32_t_INT64_t
...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT32_t_INT64_t cimport csr_to_csc_kernel_INT32_t_INT64_t, csc_to_csr_kernel_INT32_t_INT64_t, csr_to_ll_kernel_INT32_t_INT64_t
...
include "csr_mat_kernel/csr_mat_multiplication_by_numpy_vector_kernel_INT32_t_INT64_t.pxi"
include "csr_mat_helpers/csr_mat_multiplication_INT32_t_INT64_t.pxi"
include "csr_mat_helpers/csr_mat_is_symmetric_INT32_t_INT64_t.pxi"
...
cdef class CSRSparseMatrix_INT32_t_INT64_t(ImmutableSparseMatrix_INT32_t_INT64_t):
    """
    ...
    def copy(self):
        ...
        cdef CSRSparseMatrix_INT32_t_INT64_t self_copy
        ...
        self_copy = CSRSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def triu(self, int k = 0):
        ...
        if self.__store_symmetric:
            ...
            csc_to_csr_kernel_INT32_t_INT64_t(self.__nrow, self.__ncol, nnz,
    ...
    def to_csc(self):
        ...
        csr_to_csc_kernel_INT32_t_INT64_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT32_t *>self.ind, <INT32_t *>self.col, <INT64_t *>self.val,
                       ind, row, val)
    ...
    def to_ll(self):
        ...
        csr_to_ll_kernel_INT32_t_INT64_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT32_t *>self.ind, <INT32_t *>self.col, <INT64_t *>self.val,
                       root, col, link, val)

        return MakeLLSparseMatrix_INT32_t_INT64_t(nrow=self.__nrow,
                                                 ncol=self.__ncol,
                                                 nnz=self.__nnz,
                                                 free=free,
                                                 nalloc=nalloc,
    ...
    def matvec(self, b):
        ...
        return multiply_csr_mat_with_numpy_vector_INT32_t_INT64_t(self, b)
    ...
    def matvec_transp(self, b):
        ...
        """
        assert are_mixed_types_compatible(INT64_T, b.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(INT64_T)
        return multiply_transposed_csr_mat_with_numpy_vector_INT32_t_INT64_t(self, b)
    ...
    def matdot(self, B):
        ...
        # CASES
        if isinstance(B, CSCSparseMatrix_INT32_t_INT64_t):
            return multiply_csr_mat_by_csc_mat_INT32_t_INT64_t(self, B)
        else:
...
# Factory methods
########################################################################################################################
cdef MakeCSRSparseMatrix_INT32_t_INT64_t(INT32_t nrow,
                                        INT32_t ncol,
                                        INT32_t nnz,
                                        INT32_t * ind,
                                        INT32_t * col,
    ...
    """
    cdef CSRSparseMatrix_INT32_t_INT64_t csr_mat

    csr_mat = CSRSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, store_symmetric=store_symmetric, store_zero=store_zero)
