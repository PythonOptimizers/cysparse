...
cdef class CSCSparseMatrix_INT64_t_FLOAT32_t(ImmutableSparseMatrix_INT64_t_FLOAT32_t):
    ...
    def diag(self):
        ...
        cdef cnp.ndarray[cnp.npy_float32, ndim=1, mode='c'] diagonal = np.zeros(diag_size, dtype=np.float32)

        # direct access to NumPy array
        cdef FLOAT32_t * diagonal_data = <FLOAT32_t *> cnp.PyArray_DATA(diagonal)
...
cdef MakeCSCSparseMatrix_INT64_t_FLOAT32_t(INT64_t nrow, INT64_t ncol, INT64_t nnz, INT64_t * ind, INT64_t * row, FLOAT32_t * val, bint __is_symmetric):
    ...
    """
    csc_mat = CSCSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, __is_symmetric=__is_symmetric)
