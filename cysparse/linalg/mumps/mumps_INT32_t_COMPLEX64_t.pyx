...
In CySparse (MumpsContext_INT32_t_COMPLEX64_t), Xmumps_c() is called by a call to self.mumps_call().
...
from cysparse.types.cysparse_generic_types cimport split_array_complex_values_kernel_INT32_t_COMPLEX64_t, join_array_complex_values_kernel_INT32_t_COMPLEX64_t
...
cdef class MumpsContext_INT32_t_COMPLEX64_t:
    ...
    def __cinit__(self, LLSparseMatrix_INT32_t_COMPLEX64_t A, comm_fortran=-987654, verbose=False):
        """
        Args:
            A: A :class:`LLSparseMatrix_INT32_t_COMPLEX64_t` object.
        ...
        self.a_val = <COMPLEX64_t *> PyMem_Malloc(self.nnz * sizeof(COMPLEX64_t))
    ...
    #property cntl:
    #    def __get__(self):
    #        cdef CMUMPS_REAL[::1] view = <CMUMPS_REAL[::1]> self.params.cntl
    #        return view
    ...
    def solve(self, **kwargs):
        ...
        if 'rhs' in kwargs:
            ...
            assert are_mixed_types_compatible(COMPLEX64_T, rhs.dtype), "Rhs matrix or vector must have a Numpy compatible type (%s)!" % cysparse_to_numpy_type(COMPLEX64_T)
            ...
            self.solve_dense(<COMPLEX64_t *> cnp.PyArray_DATA(x), rhs_shape[0], nrhs)
        ...
        ################################################################################################################
            ...
            self.solve_sparse(<INT32_t *>cnp.PyArray_DATA(rhs_col_ptr), <INT32_t *>cnp.PyArray_DATA(rhs_row_ind), <COMPLEX64_t *> cnp.PyArray_DATA(rhs_val),
                        rhs_nnz, nrhs, <COMPLEX64_t *> cnp.PyArray_DATA(x), x_length)
        else:
