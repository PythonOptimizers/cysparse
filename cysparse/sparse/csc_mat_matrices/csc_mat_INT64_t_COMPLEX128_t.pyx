...
cdef class CSCSparseMatrix_INT64_t_COMPLEX128_t(ImmutableSparseMatrix_INT64_t_COMPLEX128_t):
    ...
    def diag(self):
        ...
        cdef cnp.ndarray[cnp.npy_complex128, ndim=1, mode='c'] diagonal = np.zeros(diag_size, dtype=np.complex128)

        # direct access to NumPy array
        cdef COMPLEX128_t * diagonal_data = <COMPLEX128_t *> cnp.PyArray_DATA(diagonal)
...
# Factory methods
########################################################################################################################
cdef MakeCSCSparseMatrix_INT64_t_COMPLEX128_t(INT64_t nrow, INT64_t ncol, INT64_t nnz, INT64_t * ind, INT64_t * row, COMPLEX128_t * val, bint __is_symmetric):
    """
    ...
    """
    csc_mat = CSCSparseMatrix_INT64_t_COMPLEX128_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, __is_symmetric=__is_symmetric)
