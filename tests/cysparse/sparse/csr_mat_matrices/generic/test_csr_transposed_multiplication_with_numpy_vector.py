...
# Tests
########################################################################################################################
class CySparseCSRTransposedMultiplicationWithANumpyVectorBaseTestCase(unittest.TestCase):
    def setUp(self):
        pass

class CySparseCSRTransposedMultiplicationWithANumpyVectorTestCase(CySparseCSRTransposedMultiplicationWithANumpyVectorBaseTestCase):
    """
    ...
    def test_simple_multiplication_one_by_one(self):
        ...
        csr_y = self.l_1_1_csr.matvec_transp(self.x_INT32_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_2.matvec_transp(self.x_INT64_t)
        csr_y = self.l_1_2_csr.matvec_transp(self.x_INT64_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_3.matvec_transp(self.x_FLOAT32_t)
        csr_y = self.l_1_3_csr.matvec_transp(self.x_FLOAT32_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_4.matvec_transp(self.x_FLOAT64_t)
        csr_y = self.l_1_4_csr.matvec_transp(self.x_FLOAT64_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_5.matvec_transp(self.x_FLOAT128_t)
        csr_y = self.l_1_5_csr.matvec_transp(self.x_FLOAT128_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_6.matvec_transp(self.x_COMPLEX64_t)
        csr_y = self.l_1_6_csr.matvec_transp(self.x_COMPLEX64_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_7.matvec_transp(self.x_COMPLEX128_t)
        csr_y = self.l_1_7_csr.matvec_transp(self.x_COMPLEX128_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_8.matvec_transp(self.x_COMPLEX256_t)
        csr_y = self.l_1_8_csr.matvec_transp(self.x_COMPLEX256_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
        ...
        csr_y = self.l_2_1_csr.matvec_transp(self.x_INT32_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_2.matvec_transp(self.x_INT64_t)
        csr_y = self.l_2_2_csr.matvec_transp(self.x_INT64_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_3.matvec_transp(self.x_FLOAT32_t)
        csr_y = self.l_2_3_csr.matvec_transp(self.x_FLOAT32_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_4.matvec_transp(self.x_FLOAT64_t)
        csr_y = self.l_2_4_csr.matvec_transp(self.x_FLOAT64_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_5.matvec_transp(self.x_FLOAT128_t)
        csr_y = self.l_2_5_csr.matvec_transp(self.x_FLOAT128_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_6.matvec_transp(self.x_COMPLEX64_t)
        csr_y = self.l_2_6_csr.matvec_transp(self.x_COMPLEX64_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_7.matvec_transp(self.x_COMPLEX128_t)
        csr_y = self.l_2_7_csr.matvec_transp(self.x_COMPLEX128_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_8.matvec_transp(self.x_COMPLEX256_t)
        csr_y = self.l_2_8_csr.matvec_transp(self.x_COMPLEX256_t)
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
...
class CySparseSymCSRTransposedMultiplicationWithANumpyVectorTestCase(CySparseCSRTransposedMultiplicationWithANumpyVectorBaseTestCase):
    """
...
class CySparseCSRTransposedMultiplicationWithAStridedNumpyVectorTestCase(CySparseCSRTransposedMultiplicationWithANumpyVectorBaseTestCase):
    """
    ...
    def test_simple_multiplication_one_by_one(self):
        ...
        l_y = self.l_1_1.matvec_transp(self.x_INT32_t)
        csr_y = self.l_1_1_csr.matvec_transp(self.x_strided_INT32_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_2.matvec_transp(self.x_INT64_t)
        csr_y = self.l_1_2_csr.matvec_transp(self.x_strided_INT64_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_3.matvec_transp(self.x_FLOAT32_t)
        csr_y = self.l_1_3_csr.matvec_transp(self.x_strided_FLOAT32_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_4.matvec_transp(self.x_FLOAT64_t)
        csr_y = self.l_1_4_csr.matvec_transp(self.x_strided_FLOAT64_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_5.matvec_transp(self.x_FLOAT128_t)
        csr_y = self.l_1_5_csr.matvec_transp(self.x_strided_FLOAT128_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_6.matvec_transp(self.x_COMPLEX64_t)
        csr_y = self.l_1_6_csr.matvec_transp(self.x_strided_COMPLEX64_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_7.matvec_transp(self.x_COMPLEX128_t)
        csr_y = self.l_1_7_csr.matvec_transp(self.x_strided_COMPLEX128_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_8.matvec_transp(self.x_COMPLEX256_t)
        csr_y = self.l_1_8_csr.matvec_transp(self.x_strided_COMPLEX256_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
        ...
        l_y = self.l_2_1.matvec_transp(self.x_INT32_t)
        csr_y = self.l_2_1_csr.matvec_transp(self.x_strided_INT32_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_2.matvec_transp(self.x_INT64_t)
        csr_y = self.l_2_2_csr.matvec_transp(self.x_strided_INT64_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_3.matvec_transp(self.x_FLOAT32_t)
        csr_y = self.l_2_3_csr.matvec_transp(self.x_strided_FLOAT32_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_4.matvec_transp(self.x_FLOAT64_t)
        csr_y = self.l_2_4_csr.matvec_transp(self.x_strided_FLOAT64_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_5.matvec_transp(self.x_FLOAT128_t)
        csr_y = self.l_2_5_csr.matvec_transp(self.x_strided_FLOAT128_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_6.matvec_transp(self.x_COMPLEX64_t)
        csr_y = self.l_2_6_csr.matvec_transp(self.x_strided_COMPLEX64_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_7.matvec_transp(self.x_COMPLEX128_t)
        csr_y = self.l_2_7_csr.matvec_transp(self.x_strided_COMPLEX128_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_8.matvec_transp(self.x_COMPLEX256_t)
        csr_y = self.l_2_8_csr.matvec_transp(self.x_strided_COMPLEX256_t[::self.stride_factor])
        for i in xrange(self.ncol):
            self.failUnless(l_y[i] == csr_y[i])
...
class CySparseSymCSRTransposedMultiplicationWithAStridedNumpyVectorTestCase(CySparseCSRTransposedMultiplicationWithANumpyVectorBaseTestCase):
    """
