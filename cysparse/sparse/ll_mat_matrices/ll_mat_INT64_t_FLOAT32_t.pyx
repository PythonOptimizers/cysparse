...
from cysparse.sparse.csr_mat_matrices.csr_mat_INT64_t_FLOAT32_t cimport MakeCSRSparseMatrix_INT64_t_FLOAT32_t
from cysparse.sparse.csc_mat_matrices.csc_mat_INT64_t_FLOAT32_t cimport MakeCSCSparseMatrix_INT64_t_FLOAT32_t
...
########################################################################################################################
from cysparse.sparse.ll_mat_views.ll_mat_view_INT64_t_FLOAT32_t cimport LLSparseMatrixView_INT64_t_FLOAT32_t, MakeLLSparseMatrixView_INT64_t_FLOAT32_t
...
cdef class LLSparseMatrix_INT64_t_FLOAT32_t(MutableSparseMatrix_INT64_t_FLOAT32_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, use_nonzero_storage=self.__use_nonzero_storage, use_symmetric_storage=self.__use_symmetric_storage)
    ...
    def create_transpose(self):
        ...
        if self.__use_symmetric_storage:
            ...
            transpose = LLSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, use_nonzero_storage=self.__use_nonzero_storage, use_symmetric_storage=self.__use_symmetric_storage)
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT64_t_FLOAT32_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind,
                                                     row=row,
    ...
    ####################################################################################################################
    #                                            ### ASSIGN ###
    cdef assign(self, LLSparseMatrixView_INT64_t_FLOAT32_t view, object obj):
        """
        Set ``A[..., ...] = obj`` directly.

        Args:
            view: An ``LLSparseMatrixView_INT64_t_FLOAT32_t`` that points to this matrix (``self``).
            obj: Can be a:
                - ``LLSparseMatrix``;
                - ``LLSparseMatrixView``;
        ...
        if self.__use_symmetric_storage:
            if PySparseMatrix_Check(obj):
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A = <LLSparseMatrix_INT64_t_FLOAT32_t> obj
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A_view = <LLSparseMatrixView_INT64_t_FLOAT32_t> obj
    ...
    def tril(self, int k):
        ...
        ll_mat_tril = LLSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, use_nonzero_storage=self.__use_nonzero_storage, use_symmetric_storage=False)
    ...
    def triu(self, int k):
        ...
        ll_mat_triu = LLSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, use_nonzero_storage=self.__use_nonzero_storage, use_symmetric_storage=False)
        ...
        if self.__use_symmetric_storage == B.__use_symmetric_storage:
            ...
            for i from 0 <= i < B.__nrow:
                ...
                while k != -1:
                    ...
                    update_ll_mat_item_add_INT64_t_FLOAT32_t(self, i, j, v)
                    if i != j:
                        update_ll_mat_item_add_INT64_t_FLOAT32_t(self, j, i, v)
                    k = B.link[k]
        else:
    ...
    def matvec(self, b):
        ...
        return multiply_ll_mat_with_numpy_vector_INT64_t_FLOAT32_t(self, b)
