...
cdef class LLSparseMatrix_INT32_t_FLOAT32_t(MutableSparseMatrix_INT32_t_FLOAT32_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT32_t_FLOAT32_t(control_object=unexposed_value, no_memory=True, nrow=self.nrow, ncol=self.ncol, size_hint=self.size_hint, store_zeros=self.__store_zeros, __is_symmetric=self.__is_symmetric)
    ...
    def create_transpose(self):
        ...
        if self.__is_symmetric:
            ...
            transpose = LLSparseMatrix_INT32_t_FLOAT32_t(control_object=unexposed_value, nrow=self.ncol, ncol=self.nrow, size_hint=self.nnz, store_zeros=self.__store_zeros, __is_symmetric=self.__is_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT32_t_FLOAT32_t(nrow=self.nrow, ncol=self.ncol, nnz=self.nnz, ind=ind, col=col, val=val, __is_symmetric=self.__is_symmetric)
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT32_t_FLOAT32_t(nrow=self.nrow, ncol=self.ncol, nnz=self.nnz, ind=ind, row=row, val=val, __is_symmetric=self.__is_symmetric)
    ...
    def update_add_at(self, id1, id2, val):
        ...
        See :meth:`update_add_at_with_numpy_arraysINT32_t_FLOAT32_t`.
        """
        return update_add_at_with_numpy_arraysINT32_t_FLOAT32_t(self, id1, id2, val)
    ...
    def print_to(self, OUT, width=9, print_big_matrices=False, transposed=False):
        ...
        if print_big_matrices or (self.nrow <= LL_MAT_PPRINT_COL_THRESH and self.ncol <= LL_MAT_PPRINT_ROW_THRESH):
            ...
            mat = <FLOAT32_t *> PyMem_Malloc(self.nrow * self.ncol * sizeof(FLOAT32_t))
