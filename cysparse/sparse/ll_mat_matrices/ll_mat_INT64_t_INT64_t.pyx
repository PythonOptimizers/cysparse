...
cdef class LLSparseMatrix_INT64_t_INT64_t(MutableSparseMatrix_INT64_t_INT64_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT64_t_INT64_t(control_object=unexposed_value, no_memory=True, nrow=self.nrow, ncol=self.ncol, size_hint=self.size_hint, store_zeros=self.__store_zeros, __is_symmetric=self.__is_symmetric)
    ...
    def create_transpose(self):
        ...
        if self.__is_symmetric:
            ...
            transpose = LLSparseMatrix_INT64_t_INT64_t(control_object=unexposed_value, nrow=self.ncol, ncol=self.nrow, size_hint=self.nnz, store_zeros=self.__store_zeros, __is_symmetric=self.__is_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT64_t_INT64_t(nrow=self.nrow, ncol=self.ncol, nnz=self.nnz, ind=ind, col=col, val=val, __is_symmetric=self.__is_symmetric)
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT64_t_INT64_t(nrow=self.nrow, ncol=self.ncol, nnz=self.nnz, ind=ind, row=row, val=val, __is_symmetric=self.__is_symmetric)
