...
cdef class CSCSparseMatrix_INT64_t_FLOAT32_t(ImmutableSparseMatrix_INT64_t_FLOAT32_t):
    ...
    def get_numpy_arrays(self):
        ...
        cdef INT64_t * ind_array = <INT64_t * > self.ind

        ind_numpy_array = cnp.PyArray_SimpleNewFromData(1, dim, cnp.NPY_INT64, ind_array)
...
# Factory methods
########################################################################################################################
cdef MakeCSCSparseMatrix_INT64_t_FLOAT32_t(INT64_t nrow, INT64_t ncol, INT64_t nnz, INT64_t * ind, INT64_t * row, FLOAT32_t * val):
    """
    ...
    """
    csc_mat = CSCSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz)
