...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT32_t_FLOAT32_t cimport csr_to_csc_kernel_INT32_t_FLOAT32_t, csc_to_csr_kernel_INT32_t_FLOAT32_t
...
cdef class CSRSparseMatrix_INT32_t_FLOAT32_t(ImmutableSparseMatrix_INT32_t_FLOAT32_t):
    ...
    def copy(self):
        ...
        self_copy = CSRSparseMatrix_INT32_t_FLOAT32_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zeros=self.__store_zeros, is_symmetric=self.__is_symmetric)
    ...
    def tril(self, int k):
        ...
        return MakeCSRSparseMatrix_INT32_t_FLOAT32_t(self.__nrow, self.__ncol, nnz, ind, col, val, is_symmetric=False, store_zeros=self.__store_zeros)
    ...
    def to_csc(self):
        ...
        return MakeCSCSparseMatrix_INT32_t_FLOAT32_t(self.__nrow, self.__ncol, self.__nnz, ind, row, val, is_symmetric=self.is_symmetric, store_zeros=self.store_zeros)
    ...
    def __mul__(self, other):
        ...
        # test if implemented
        if isinstance(other, (MutableSparseMatrix_INT32_t_FLOAT32_t, ImmutableSparseMatrix_INT32_t_FLOAT32_t)):
            pass
        else:
            raise NotImplemented("Multiplication not (yet) allowed")

        # CASES
        if isinstance(other, CSCSparseMatrix_INT32_t_FLOAT32_t):
            return multiply_csr_mat_by_csc_mat_INT32_t_FLOAT32_t(self, other)
        else:
            raise NotImplemented("Multiplication not (yet) allowed")
