...
cdef class LLSparseMatrix_INT64_t_INT64_t(MutableSparseMatrix_INT64_t_INT64_t):
    ...
    def create_transpose(self):
        ...
        if self.__is_symmetric:
            ...
            transpose = LLSparseMatrix_INT64_t_INT64_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.nnz, store_zeros=self.__store_zeros, __is_symmetric=self.__is_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT64_t_INT64_t(nrow=self.__nrow, ncol=self.__ncol, nnz=self.nnz, ind=ind, col=col, val=val, __is_symmetric=self.__is_symmetric)
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT64_t_INT64_t(nrow=self.__nrow, ncol=self.__ncol, nnz=self.nnz, ind=ind, row=row, val=val, __is_symmetric=self.__is_symmetric)
    ...
    def update_add_at(self, id1, id2, val):
        ...
        See :meth:`update_add_at_with_numpy_arraysINT64_t_INT64_t`.
        """
        return update_add_at_with_numpy_arraysINT64_t_INT64_t(self, id1, id2, val)
