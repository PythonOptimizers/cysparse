...
from cysparse.sparse.ll_mat_matrices.ll_mat_INT64_t_INT64_t cimport LLSparseMatrix_INT64_t_INT64_t, MakeLLSparseMatrix_INT64_t_INT64_t
...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT64_t_INT64_t cimport csr_to_csc_kernel_INT64_t_INT64_t, csc_to_csr_kernel_INT64_t_INT64_t, csc_to_ll_kernel_INT64_t_INT64_t

from cysparse.sparse.csr_mat_matrices.csr_mat_INT64_t_INT64_t cimport CSRSparseMatrix_INT64_t_INT64_t, MakeCSRSparseMatrix_INT64_t_INT64_t
...
include "csc_mat_kernel/csc_mat_multiplication_by_numpy_vector_kernel_INT64_t_INT64_t.pxi"
include "csc_mat_helpers/csc_mat_multiplication_INT64_t_INT64_t.pxi"
include "csc_mat_helpers/csc_mat_is_symmetric_INT64_t_INT64_t.pxi"

cdef class CSCSparseMatrix_INT64_t_INT64_t(ImmutableSparseMatrix_INT64_t_INT64_t):
    """
    ...
    def copy(self):
        ...
        cdef CSCSparseMatrix_INT64_t_INT64_t self_copy
        ...
        self_copy = CSCSparseMatrix_INT64_t_INT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def tril(self, int k = 0):
        ...
        return MakeCSCSparseMatrix_INT64_t_INT64_t(self.__nrow,
                                                  self.__ncol,
                                                  nnz,
                                                  ind,
                                                  row,
    ...
    def triu(self, int k = 0):
        ...
        if self.__store_symmetric:
            ...
            csr_to_csc_kernel_INT64_t_INT64_t(self.__nrow, self.__ncol, nnz,
    ...
    def to_csr(self):
        ...
        csc_to_csr_kernel_INT64_t_INT64_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.row, <INT64_t *>self.val,
                       ind, col, val)

        return MakeCSRSparseMatrix_INT64_t_INT64_t(self.__nrow,
                                                  self.__ncol,
                                                  self.__nnz,
                                                  ind,
                                                  col,
    ...
    def to_ll(self):
        ...
        cdef INT64_t * val = <INT64_t *> PyMem_Malloc(nalloc * sizeof(INT64_t))
        if not val:
            PyMem_Free(root)
            PyMem_Free(col)
            raise MemoryError()
        ...
        csc_to_ll_kernel_INT64_t_INT64_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.row, <INT64_t *>self.val,
                       root, col, link, val)

        return MakeLLSparseMatrix_INT64_t_INT64_t(nrow=self.__nrow,
                                                 ncol=self.__ncol,
                                                 nnz=self.__nnz,
                                                 free=free,
                                                 nalloc=nalloc,
...
# Factory methods
########################################################################################################################
cdef MakeCSCSparseMatrix_INT64_t_INT64_t(INT64_t nrow,
                                        INT64_t ncol,
                                        INT64_t nnz,
                                        INT64_t * ind,
                                        INT64_t * row,
    ...
    """
    cdef CSCSparseMatrix_INT64_t_INT64_t csc_mat
    csc_mat = CSCSparseMatrix_INT64_t_INT64_t(control_object=unexposed_value,
                                             nrow=nrow,
                                             ncol=ncol,
                                             nnz=nnz,
