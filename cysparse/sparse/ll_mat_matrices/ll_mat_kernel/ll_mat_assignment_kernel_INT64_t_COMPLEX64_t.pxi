...
cdef bint update_ll_mat_item_add_INT64_t_COMPLEX64_t(LLSparseMatrix_INT64_t_COMPLEX64_t A, INT64_t i, INT64_t j, COMPLEX64_t x):
    """
    Update-add matrix entry: ``A[i,j] += x``

    Args:
        A: :class:`LLSparseMatrix_INT64_t_COMPLEX64_t` to update.
        i, j: Coordinates of item to update.
        x (COMPLEX64_t): Value to add to element ``(i, j)`` to update.
