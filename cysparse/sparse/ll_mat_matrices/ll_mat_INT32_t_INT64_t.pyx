...
from cysparse.sparse.ll_mat_views.ll_mat_view_INT32_t_INT64_t cimport PyLLSparseMatrixView_Check
...
from cysparse.sparse.csr_mat_matrices.csr_mat_INT32_t_INT64_t cimport MakeCSRSparseMatrix_INT32_t_INT64_t
from cysparse.sparse.csc_mat_matrices.csc_mat_INT32_t_INT64_t cimport MakeCSCSparseMatrix_INT32_t_INT64_t
...
include "ll_mat_helpers/ll_mat_is_symmetric_INT32_t_INT64_t.pxi"
...
cdef class LLSparseMatrix_INT32_t_INT64_t(MutableSparseMatrix_INT32_t_INT64_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def create_transpose(self):
        ...
        cdef:
            INT32_t i, j, k
            LLSparseMatrix_INT32_t_INT64_t transpose

        if self.__store_symmetric:
            return self.copy()
        else:
            transpose = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT32_t_INT64_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind, col=col,
                                                     val=val,
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT32_t_INT64_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind,
                                                     row=row,
    ...
    cdef assign(self, LLSparseMatrixView_INT32_t_INT64_t view, object obj):
        ...
        cdef:
            ...
            LLSparseMatrixView_INT32_t_INT64_t A_view
        ...
        if self.__store_symmetric:
            if PySparseMatrix_Check(obj):
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A = <LLSparseMatrix_INT32_t_INT64_t> obj
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A_view = <LLSparseMatrixView_INT32_t_INT64_t> obj
        ...
        if PyList_Check(<PyObject *>id1) and PyList_Check(<PyObject *>id2):
            ...
            if PyList_Check(<PyObject *>b):
                ...
                for i_list from 0 <= i_list < id1_list_length:
                    ...
                    self.safe_put(PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id1, i_list)), PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id2, i_list)), <INT64_t> PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>b, i_list)))
            ...
            ############################################################################################################
                ...
                if cnp.PyArray_ISCONTIGUOUS(b):
                    for i_list from 0 <= i_list < id1_list_length:
                        ...
                        self.safe_put(PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id1, i_list)), PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id2, i_list)), <INT64_t> b_data[i_list])
                else:
                    for i_list from 0 <= i_list < id1_list_length:
                        self.safe_put(PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id1, i_list)), PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id2, i_list)), <INT64_t> b_data[i_list * b_data_incx])
        ...
        ################################################################################################################
            ...
            if PyList_Check(<PyObject *>b):
                ...
                for i_list from 0 <= i_list < b_list_length:
                    ...
                    self.safe_put(id1_data[i_list], id2_data[i_list], <INT64_t> PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>b, i_list)))
            ...
            ############################################################################################################
                ...
                if cnp.PyArray_ISCONTIGUOUS(b):
                    for i_array from 0 <= i_array < id1_array_length:
                        self.safe_put(id1_data[i_array], id2_data[i_array], <INT64_t> b_data[i_array])
                else:
                    for i_array from 0 <= i_array < id1_array_length:
                        self.safe_put(id1_data[i_array], id2_data[i_array], <INT64_t> b_data[i_array * b_data_incx])
    ...
    cdef fill_triplet(self, INT32_t * a_row, INT32_t * a_col, INT64_t * a_val):
        """
        Warning:
    ...
    def tril(self, int k = 0):
        ...
        ll_mat_tril = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    def triu(self, int k = 0):
        ...
        ll_mat_triu = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    def update_add_at(self, id1, id2, val):
        ...
        See :meth:`update_add_at_with_numpy_arrays_INT32_t_INT64_t`.
        """
        return update_add_at_with_numpy_arrays_INT32_t_INT64_t(self, id1, id2, val)
    ...
    def matdot(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            ...
            if B.ndim == 2:
                return multiply_ll_mat_with_numpy_ndarray_INT64_t(self, B)
            else:
    ...
    def matdot_transp(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            ...
            return multiply_transposed_ll_mat_by_ll_mat_INT32_t_INT64_t(self, B)
        elif cnp.PyArray_Check(B):
            assert are_mixed_types_compatible(INT64_T, B.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(INT64_T)
            return multiply_transposed_ll_mat_with_numpy_ndarray_INT64_t(self, B)
        else:
            raise NotImplementedError("Multiplication with this kind of object not allowed")
...
cdef MakeLLSparseMatrix_INT32_t_INT64_t(INT32_t nrow,
    ...
    """
    cdef LLSparseMatrix_INT32_t_INT64_t ll_mat

    ll_mat = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value,
                                           no_memory=True,
                                           size_hint=nalloc,
                                           nrow=nrow,
                                           ncol=ncol,
