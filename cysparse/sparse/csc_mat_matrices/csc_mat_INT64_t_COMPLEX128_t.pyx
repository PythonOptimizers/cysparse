...
cdef class CSCSparseMatrix_INT64_t_COMPLEX128_t(ImmutableSparseMatrix_INT64_t_COMPLEX128_t):
    ...
    def triu(self, int k):
        ...
        cdef COMPLEX128_t * csr_val = <COMPLEX128_t *> PyMem_Malloc(self.__nnz * sizeof(COMPLEX128_t))
        if not csr_val:
            PyMem_Free(ind)
            PyMem_Free(row)
            PyMem_Free(val)
...
# Factory methods
########################################################################################################################
cdef MakeCSCSparseMatrix_INT64_t_COMPLEX128_t(INT64_t nrow, INT64_t ncol, INT64_t nnz, INT64_t * ind, INT64_t * row, COMPLEX128_t * val, bint is_symmetric, bint store_zeros):
    """
    ...
    """
    csc_mat = CSCSparseMatrix_INT64_t_COMPLEX128_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, is_symmetric=is_symmetric, store_zeros=store_zeros)
