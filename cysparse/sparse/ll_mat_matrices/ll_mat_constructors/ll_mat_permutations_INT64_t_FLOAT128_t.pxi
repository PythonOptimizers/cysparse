...
cdef LLSparseMatrix_INT64_t_FLOAT128_t MakePermutationLLSparseMatrix_INT64_t_FLOAT128_t(LLSparseMatrix_INT64_t_FLOAT128_t A, cnp.ndarray numpy_vector):
    """
    Populate an ``LLSparseMatrix_INT64_t_FLOAT128_t as a permutation matrix given a vector of permutation.

    See https://en.wikipedia.org/wiki/Permutation_matrix.

    Args:
        A: A ``LLSparseMatrix_INT64_t_FLOAT128_t`` matrix.
    ...
    for i from 0 <= i < np_vec_size:
        j = <FLOAT128_t> numpy_vector[i]

        A.put(i, j, <FLOAT128_t> 1.0)
