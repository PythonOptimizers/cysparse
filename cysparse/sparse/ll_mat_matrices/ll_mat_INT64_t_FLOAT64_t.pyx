...
cdef class LLSparseMatrix_INT64_t_FLOAT64_t(MutableSparseMatrix_INT64_t_FLOAT64_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, store_zeros=self.__store_zeros, is_symmetric=self.__is_symmetric)
    ...
    def delete_cols(self, B):
        ...
        col_indices = create_c_array_indices_from_python_object_INT64_t(self.__ncol, <PyObject *> B, &ncol)
    ...
    def create_transpose(self):
        ...
        if self.__is_symmetric:
            ...
            transpose = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, store_zeros=self.__store_zeros, is_symmetric=self.__is_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT64_t_FLOAT64_t(nrow=self.__nrow, ncol=self.__ncol, nnz=self.__nnz, ind=ind, col=col, val=val, is_symmetric=self.__is_symmetric, store_zeros=self.__store_zeros)
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT64_t_FLOAT64_t(nrow=self.__nrow, ncol=self.__ncol, nnz=self.__nnz, ind=ind, row=row, val=val, is_symmetric=self.__is_symmetric, store_zeros=self.__store_zeros)
    ...
    cdef assign(self, LLSparseMatrixView_INT64_t_FLOAT64_t view, object obj):
        ...
        if self.__is_symmetric:
            if PySparseMatrix_Check(obj):
                for i from 0 <= i < nrow:
                    for j from 0 <= j <= i:
                        self.put(row_indices[i], col_indices[j], <FLOAT64_t> obj.at(i, j))
    ...
    def triu(self, include_diagonal = True):
        ...
        ll_mat_triu = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zeros=self.__store_zeros, is_symmetric=False)
    ...
    def tril(self, include_diagonal = True):
        ...
        ll_mat_tril = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zeros=self.__store_zeros, is_symmetric=False)
    ...
    def matvec2(self, b):
        ...
        return multiply_ll_mat_with_numpy_vector2_INT64_t_FLOAT64_t(self, b)
