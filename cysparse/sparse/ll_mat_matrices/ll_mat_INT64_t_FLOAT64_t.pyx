...
from cysparse.sparse.ll_mat_views.ll_mat_view_INT64_t_FLOAT64_t cimport PyLLSparseMatrixView_Check
...
from cysparse.sparse.csr_mat_matrices.csr_mat_INT64_t_FLOAT64_t cimport MakeCSRSparseMatrix_INT64_t_FLOAT64_t
from cysparse.sparse.csc_mat_matrices.csc_mat_INT64_t_FLOAT64_t cimport MakeCSCSparseMatrix_INT64_t_FLOAT64_t
...
include "ll_mat_kernel/ll_mat_assignment_kernel_INT64_t_FLOAT64_t.pxi"
include "ll_mat_kernel/ll_mat_multiplication_by_numpy_vector_kernel_INT64_t_FLOAT64_t.pxi"
include "ll_mat_helpers/ll_mat_multiplication_INT64_t_FLOAT64_t.pxi"
include "ll_mat_helpers/ll_mat_addition_INT64_t_FLOAT64_t.pxi"
include "ll_mat_helpers/ll_mat_is_symmetric_INT64_t_FLOAT64_t.pxi"
...
########################################################################################################################
from cysparse.sparse.ll_mat_views.ll_mat_view_INT64_t_FLOAT64_t cimport LLSparseMatrixView_INT64_t_FLOAT64_t, MakeLLSparseMatrixView_INT64_t_FLOAT64_t
...
cdef class LLSparseMatrix_INT64_t_FLOAT64_t(MutableSparseMatrix_INT64_t_FLOAT64_t):
    ...
    def copy(self):
        ...
        cdef LLSparseMatrix_INT64_t_FLOAT64_t self_copy

        # we copy manually the C-arrays
        self_copy = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
        ...
        val = <FLOAT64_t *> PyMem_Malloc(self.nalloc * sizeof(FLOAT64_t))
        if not val:
            raise MemoryError()
        memcpy(val, self.val, self.nalloc * sizeof(FLOAT64_t))
        self_copy.val = val
    ...
    def create_transpose(self):
        ...
        if self.__store_symmetric:
            ...
            transpose = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT64_t_FLOAT64_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind, col=col,
                                                     val=val,
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT64_t_FLOAT64_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind,
                                                     row=row,
    ...
    cdef assign(self, LLSparseMatrixView_INT64_t_FLOAT64_t view, object obj):
        ...
        Args:
            view: An ``LLSparseMatrixView_INT64_t_FLOAT64_t`` that points to this matrix (``self``).
            obj: Can be a:
                - ``LLSparseMatrix``;
                - ``LLSparseMatrixView``;
        ...
        cdef:
            ...
            LLSparseMatrixView_INT64_t_FLOAT64_t A_view
        ...
        if self.__store_symmetric:
            if PySparseMatrix_Check(obj):
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A = <LLSparseMatrix_INT64_t_FLOAT64_t> obj
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A_view = <LLSparseMatrixView_INT64_t_FLOAT64_t> obj
                ...
                for i from 0 <= i < nrow:
                    for j from 0 <= j <= i:
                        self.put(row_indices[i], col_indices[j], <FLOAT64_t> obj)
            else:
    ...
    cdef safe_put(self, INT64_t i, INT64_t j, FLOAT64_t value):
        """
        ...
        if PyList_Check(<PyObject *>id1) and PyList_Check(<PyObject *>id2):
            ...
            if PyList_Check(<PyObject *>b):
                ...
                for i_list from 0 <= i_list < id1_list_length:
                    ...
                    self.safe_put(PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id1, i_list)), PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id2, i_list)), <FLOAT64_t> PyFloat_AsDouble(PyList_GET_ITEM(<PyObject *>b, i_list)))
            ...
            ############################################################################################################
                ...
                if not are_mixed_types_compatible(FLOAT64_T, b.dtype):
                    raise TypeError('Value NumPy array must contain elements of the right index type (%s)' % cysparse_to_numpy_type(FLOAT64_T))
                ...
                b_data = <FLOAT64_t *> cnp.PyArray_DATA(b)
                ...
                if cnp.PyArray_ISCONTIGUOUS(b):
                    ...
                    for i_list from 0 <= i_list < id1_list_length:
                        self.safe_put(PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id1, i_list)), PyInt_AS_LONG(PyList_GET_ITEM(<PyObject *>id2, i_list)), <FLOAT64_t> b_data[i_list * b_data_incx])
        ...
        ################################################################################################################
            ...
            if PyList_Check(<PyObject *>b):
                ...
                for i_list from 0 <= i_list < b_list_length:
                    ...
                    self.safe_put(id1_data[i_list], id2_data[i_list], <FLOAT64_t> PyFloat_AS_DOUBLE(PyList_GET_ITEM(<PyObject *>b, i_list)))
            ...
            ############################################################################################################
                ...
                # check if NumPy array type is compatible
                if not are_mixed_types_compatible(FLOAT64_T, b.dtype):
                    raise TypeError('Value NumPy array must contain elements of the right value type (%s)' % cysparse_to_numpy_type(FLOAT64_T))
                ...
                if cnp.PyArray_ISCONTIGUOUS(b):
                    for i_array from 0 <= i_array < id1_array_length:
                        self.safe_put(id1_data[i_array], id2_data[i_array], <FLOAT64_t> b_data[i_array])
                else:
                    for i_array from 0 <= i_array < id1_array_length:
                        self.safe_put(id1_data[i_array], id2_data[i_array], <FLOAT64_t> b_data[i_array * b_data_incx])
    ...
    cdef fill_triplet(self, INT64_t * a_row, INT64_t * a_col, FLOAT64_t * a_val):
        """
        Warning:
    ...
    def tril(self, int k = 0):
        ...
        cdef:
            INT64_t i, j, k_
            LLSparseMatrix_INT64_t_FLOAT64_t ll_mat_tril

        ll_mat_tril = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    def triu(self, int k = 0):
        ...
        cdef:
            INT64_t i, j, k_
            LLSparseMatrix_INT64_t_FLOAT64_t ll_mat_triu

        ll_mat_triu = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    def update_add_at(self, id1, id2, val):
        ...
        See :meth:`update_add_at_with_numpy_arrays_INT64_t_FLOAT64_t`.
        """
        return update_add_at_with_numpy_arrays_INT64_t_FLOAT64_t(self, id1, id2, val)
    ...
    def matdot(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            ...
            if B.ndim == 2:
                return multiply_ll_mat_with_numpy_ndarray_FLOAT64_t(self, B)
            else:
    ...
    def matdot_transp(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            ...
            return multiply_transposed_ll_mat_by_ll_mat_INT64_t_FLOAT64_t(self, B)
        elif cnp.PyArray_Check(B):
            assert are_mixed_types_compatible(FLOAT64_T, B.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(FLOAT64_T)
            return multiply_transposed_ll_mat_with_numpy_ndarray_FLOAT64_t(self, B)
        else:
            raise NotImplementedError("Multiplication with this kind of object not allowed")
...
# Factory methods
########################################################################################################################
cdef MakeLLSparseMatrix_INT64_t_FLOAT64_t(INT64_t nrow,
                                        INT64_t ncol,
                                        INT64_t nnz,
                                        INT64_t free,
                                        INT64_t nalloc,
    ...
    """
    cdef LLSparseMatrix_INT64_t_FLOAT64_t ll_mat

    ll_mat = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value,
                                           no_memory=True,
                                           size_hint=nalloc,
                                           nrow=nrow,
                                           ncol=ncol,
