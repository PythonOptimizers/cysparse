...
cdef update_add_at_with_numpy_arrays_INT64_t_COMPLEX256_t(LLSparseMatrix_INT64_t_COMPLEX256_t A,
                                                   cnp.ndarray[cnp.npy_int64, ndim=1, mode='c'] id1,
                                                   cnp.ndarray[cnp.npy_int64, ndim=1, mode='c'] id2,
                                                   cnp.ndarray[cnp.npy_complex256, ndim=1] val):
    """
    ...
    cdef COMPLEX256_t * val_data = <COMPLEX256_t *> cnp.PyArray_DATA(val)
    ...
    if cnp.PyArray_ISCONTIGUOUS(val):
        for i from 0<= i < array_size:
            update_ll_mat_item_add_INT64_t_COMPLEX256_t(A, id1_data[i], id2_data[i], val_data[i])
    else:
        # val is not C-contiguous
        for i from 0<= i < array_size:
            update_ll_mat_item_add_INT64_t_COMPLEX256_t(A, id1_data[i], id2_data[i], val_data[i * incx])
