...
cdef LLSparseMatrix_INT64_t_FLOAT64_t multiply_transposed_ll_mat_with_self(LLSparseMatrix_INT64_t_FLOAT64_t A):
    ...
    C = LLSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, nrow=A.ncol, ncol=A.ncol, size_hint=size_hint, store_zeros=store_zeros, is_symmetric=True)
...
######################
cdef cnp.ndarray[cnp.npy_float64, ndim=1, mode='c'] multiply_ll_mat_with_numpy_vector2_INT64_t_FLOAT64_t(LLSparseMatrix_INT64_t_FLOAT64_t A, cnp.ndarray[cnp.npy_float64, ndim=1, mode='c'] b):
    """
    Multiply a :class:`LLSparseMatrix` ``A`` with a numpy vector ``b``.
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__is_symmetric:
            ...
            multiply_ll_mat_with_numpy_vector_kernel2_INT64_t_FLOAT64_t(A_nrow, b_data, c_data, A.val, A.col, A.link, A.root)
    else:
        pass
        #if A.__is_symmetric:
        #    multiply_sym_ll_mat_with_strided_numpy_vector_kernel_INT64_t_FLOAT64_t(A.nrow,
