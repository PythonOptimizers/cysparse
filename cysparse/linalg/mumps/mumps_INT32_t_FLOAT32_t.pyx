...
<SMUMPS_COMPLEX *> can be used for **ALL** four types.
...
from cysparse.linalg.mumps.mumps_INT32_t_FLOAT32_t cimport BaseMUMPSContext_INT32_FLOAT32, c_to_fortran_index_array, MUMPS_INT
...
cpdef MUMPSContext_INT32_t_FLOAT32_t(LLSparseMatrix_INT32_t_FLOAT32_t A, comm_fortran=-987654, verbose=False):
    """
    MUMPS Context.
    ...
    aval = <FLOAT32_t *> PyMem_Malloc(nnz * sizeof(FLOAT32_t))
    A.take_triplet_pointers(arow, acol, aval)


    context = BaseMUMPSContext_INT32_FLOAT32(n, nnz, comm_fortran, sym, verbose)
