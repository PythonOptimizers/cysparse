...
from cysparse.sparse.s_mat_matrices.s_mat_INT64_t_FLOAT32_t cimport ImmutableSparseMatrix_INT64_t_FLOAT32_t
...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT64_t_FLOAT32_t cimport csr_to_csc_kernel_INT64_t_FLOAT32_t, csc_to_csr_kernel_INT64_t_FLOAT32_t, csc_to_ll_kernel_INT64_t_FLOAT32_t

from cysparse.sparse.csr_mat_matrices.csr_mat_INT64_t_FLOAT32_t cimport CSRSparseMatrix_INT64_t_FLOAT32_t, MakeCSRSparseMatrix_INT64_t_FLOAT32_t
...
include "csc_mat_kernel/csc_mat_multiplication_by_numpy_vector_kernel_INT64_t_FLOAT32_t.pxi"
include "csc_mat_helpers/csc_mat_multiplication_INT64_t_FLOAT32_t.pxi"
include "csc_mat_helpers/csc_mat_is_symmetric_INT64_t_FLOAT32_t.pxi"

cdef class CSCSparseMatrix_INT64_t_FLOAT32_t(ImmutableSparseMatrix_INT64_t_FLOAT32_t):
    """
    ...
    def copy(self):
        ...
        cdef CSCSparseMatrix_INT64_t_FLOAT32_t self_copy
        ...
        self_copy = CSCSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def tril(self, int k = 0):
        ...
        temp = <FLOAT32_t *> PyMem_Realloc(val, nnz * sizeof(FLOAT32_t))
        val = <FLOAT32_t*>temp

        return MakeCSCSparseMatrix_INT64_t_FLOAT32_t(self.__nrow,
                                                  self.__ncol,
                                                  nnz,
                                                  ind,
                                                  row,
    ...
    def triu(self, int k = 0):
        ...
        if self.__store_symmetric:
            ...
            csr_val = <FLOAT32_t *> PyMem_Malloc(self.__nnz * sizeof(FLOAT32_t))
            if not csr_val:
                PyMem_Free(ind)
                PyMem_Free(row)
                PyMem_Free(val)
            ...
            csr_to_csc_kernel_INT64_t_FLOAT32_t(self.__nrow, self.__ncol, nnz,
    ...
    def to_csr(self):
        ...
        csc_to_csr_kernel_INT64_t_FLOAT32_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.row, <FLOAT32_t *>self.val,
                       ind, col, val)

        return MakeCSRSparseMatrix_INT64_t_FLOAT32_t(self.__nrow,
                                                  self.__ncol,
                                                  self.__nnz,
                                                  ind,
                                                  col,
    ...
    def to_ll(self):
        ...
        csc_to_ll_kernel_INT64_t_FLOAT32_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.row, <FLOAT32_t *>self.val,
                       root, col, link, val)
    ...
    def matvec(self, b):
        ...
        return multiply_csc_mat_with_numpy_vector_INT64_t_FLOAT32_t(self, b)
    ...
    def matvec_transp(self, b):
        ...
        """
        assert are_mixed_types_compatible(FLOAT32_T, b.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(FLOAT32_T)
        return multiply_transposed_csc_mat_with_numpy_vector_INT64_t_FLOAT32_t(self, b)
    ...
    def get_numpy_arrays(self):
        ...
        # val
        dim[0] = self.nnz
        val_numpy_array = cnp.PyArray_SimpleNewFromData(1, dim, cnp.NPY_FLOAT32, <FLOAT32_t *>self.val)
...
# Factory methods
########################################################################################################################
cdef MakeCSCSparseMatrix_INT64_t_FLOAT32_t(INT64_t nrow,
                                        INT64_t ncol,
                                        INT64_t nnz,
                                        INT64_t * ind,
                                        INT64_t * row,
    ...
    """
    cdef CSCSparseMatrix_INT64_t_FLOAT32_t csc_mat
    csc_mat = CSCSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value,
                                             nrow=nrow,
                                             ncol=ncol,
                                             nnz=nnz,
