...
cdef LLSparseMatrix_INT32_t_FLOAT64_t MakePermutationLLSparseMatrix_INT32_t_FLOAT64_t(LLSparseMatrix_INT32_t_FLOAT64_t A, cnp.ndarray numpy_vector):
    """
    Populate an ``LLSparseMatrix_INT32_t_FLOAT64_t as a permutation matrix given a vector of permutation.

    See https://en.wikipedia.org/wiki/Permutation_matrix.

    Args:
        A: A ``LLSparseMatrix_INT32_t_FLOAT64_t`` matrix.
    ...
    for i from 0 <= i < np_vec_size:
        j = <FLOAT64_t> numpy_vector[i]

        A.put(i, j, <FLOAT64_t> 1.0)
