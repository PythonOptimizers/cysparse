...
class CySparseCommonAttributesMatricesProxies_@class@_@index@_@type@_TestCase(unittest.TestCase):
    def setUp(self):
        self.A = LinearFillLLSparseMatrix(nrow=10, ncol=14, dtype=@type|type2enum@, itype=@index|type2enum@)
{% if class == 'LLSparseMatrixView' %}
        self.C = self.A[:,:]
{% else %}
YOU HAVE TO ADAPT THIS TEST TO A NEW MATRIX VIEW
{% endif %}
...
class CySparseCommonAttributesSymmetricMatricesProxies_@class@_@index@_@type@_TestCase(unittest.TestCase):
    def setUp(self):
...
class CySparseCommonAttributesWithZeroMatricesProxies_@class@_@index@_@type@_TestCase(unittest.TestCase):
    def setUp(self):
...
class CySparseCommonAttributesSymmetricWithZeroMatricesProxies_@class@_@index@_@type@_TestCase(unittest.TestCase):
    def setUp(self):
