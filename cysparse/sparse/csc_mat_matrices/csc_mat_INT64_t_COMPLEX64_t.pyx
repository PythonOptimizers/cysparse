...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT64_t_COMPLEX64_t cimport csr_to_csc_kernel_INT64_t_COMPLEX64_t
...
cdef class CSCSparseMatrix_INT64_t_COMPLEX64_t(ImmutableSparseMatrix_INT64_t_COMPLEX64_t):
    ...
    def triu(self, int k):
        ...
        cdef COMPLEX64_t * csr_val = <COMPLEX64_t *> PyMem_Malloc(self.__nnz * sizeof(COMPLEX64_t))
        if not csr_val:
            PyMem_Free(ind)
            PyMem_Free(row)
            PyMem_Free(val)
