...
# A^h * b
######################
cdef cnp.ndarray[cnp.npy_float64, ndim=1, mode='c'] multiply_conjugate_transposed_ll_mat_with_numpy_vector_INT32_t_FLOAT64_t(LLSparseMatrix_INT32_t_FLOAT64_t A, cnp.ndarray[cnp.npy_float64, ndim=1] b):
    """
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__store_symmetric:
            multiply_conjugate_tranposed_sym_ll_mat_with_numpy_vector_kernel_INT32_t_FLOAT64_t(A_nrow, A_ncol, b_data, c_data, A.val, A.col, A.link, A.root)
        else:
            multiply_conjugate_tranposed_ll_mat_with_numpy_vector_kernel_INT32_t_FLOAT64_t(A_nrow, A_ncol, b_data, c_data,
         A.val, A.col, A.link, A.root)
    else:
        if A.__store_symmetric:
            multiply_conjugate_tranposed_sym_ll_mat_with_strided_numpy_vector_kernel_INT32_t_FLOAT64_t(A_nrow, A_ncol,
                                                                 b_data, b.strides[0] / sd,
                                                                 c_data, c.strides[0] / sd,
                                                                 A.val, A.col, A.link, A.root)
        else:
            multiply_conjugate_tranposed_ll_mat_with_strided_numpy_vector_kernel_INT32_t_FLOAT64_t(A_nrow, A_ncol,
                                                                                      b_data, b.strides[0] / sd,
                                                                                      c_data, c.strides[0] / sd,
                                                                                      A.val, A.col, A.link, A.root)
...
# conj(A) * b
######################
cdef cnp.ndarray[cnp.npy_float64, ndim=1, mode='c'] multiply__conjugate_ll_mat_with_numpy_vector_INT32_t_FLOAT64_t(LLSparseMatrix_INT32_t_FLOAT64_t A, cnp.ndarray[cnp.npy_float64, ndim=1] b):
    """
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__store_symmetric:
            multiply_conjugate_sym_ll_mat_with_numpy_vector_kernel_INT32_t_FLOAT64_t(A_nrow, b_data, c_data, A.val, A.col, A.link, A.root)
        else:
            multiply_conjugate_ll_mat_with_numpy_vector_kernel_INT32_t_FLOAT64_t(A_nrow, b_data, c_data, A.val, A.col, A.link, A.root)
    else:
        if A.__store_symmetric:
            multiply_conjugate_sym_ll_mat_with_strided_numpy_vector_kernel_INT32_t_FLOAT64_t(A.nrow,
                                                                 b_data, b.strides[0] / sd,
                                                                 c_data, c.strides[0] / sd,
                                                                 A.val, A.col, A.link, A.root)
        else:
            multiply_conjugate_ll_mat_with_strided_numpy_vector_kernel_INT32_t_FLOAT64_t(A.nrow,
                                                             b_data, b.strides[0] / sd,
                                                             c_data, c.strides[0] / sd,
                                                             A.val, A.col, A.link, A.root)
