...
cdef bint update_ll_mat_item_add_INT64_t_INT32_t(LLSparseMatrix_INT64_t_INT32_t A, INT64_t i, INT64_t j, INT32_t x):
    """
    Update-add matrix entry: ``A[i,j] += x``

    Args:
        A: :class:`LLSparseMatrix_INT64_t_INT32_t` to update.
        i, j: Coordinates of item to update.
        x (INT32_t): Value to add to element ``(i, j)`` to update.
