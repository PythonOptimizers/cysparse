...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT64_t_FLOAT64_t cimport csr_to_csc_kernel_INT64_t_FLOAT64_t, csc_to_csr_kernel_INT64_t_FLOAT64_t, csc_to_ll_kernel_INT64_t_FLOAT64_t
...
cdef class CSCSparseMatrix_INT64_t_FLOAT64_t(ImmutableSparseMatrix_INT64_t_FLOAT64_t):
    ...
    def copy(self):
        ...
        cdef CSCSparseMatrix_INT64_t_FLOAT64_t self_copy
        ...
        self_copy = CSCSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def triu(self, int k = 0):
        ...
        if self.__store_symmetric:
            ...
            csr_val = <FLOAT64_t *> PyMem_Malloc(self.__nnz * sizeof(FLOAT64_t))
            if not csr_val:
                PyMem_Free(ind)
                PyMem_Free(row)
                PyMem_Free(val)
            ...
            csr_to_csc_kernel_INT64_t_FLOAT64_t(self.__nrow, self.__ncol, nnz,
    ...
    def to_csr(self):
        ...
        csc_to_csr_kernel_INT64_t_FLOAT64_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.row, <FLOAT64_t *>self.val,
                       ind, col, val)
    ...
    def to_ll(self):
        ...
        csc_to_ll_kernel_INT64_t_FLOAT64_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.row, <FLOAT64_t *>self.val,
                       root, col, link, val)
    ...
    def matvec(self, b):
        ...
        return multiply_csc_mat_with_numpy_vector_INT64_t_FLOAT64_t(self, b)
    ...
    def matvec_transp(self, b):
        ...
        """
        assert are_mixed_types_compatible(FLOAT64_T, b.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(FLOAT64_T)
        return multiply_transposed_csc_mat_with_numpy_vector_INT64_t_FLOAT64_t(self, b)
...
cdef MakeCSCSparseMatrix_INT64_t_FLOAT64_t(INT64_t nrow,
    ...
    """
    cdef CSCSparseMatrix_INT64_t_FLOAT64_t csc_mat
    csc_mat = CSCSparseMatrix_INT64_t_FLOAT64_t(control_object=unexposed_value,
                                             nrow=nrow,
                                             ncol=ncol,
                                             nnz=nnz,
