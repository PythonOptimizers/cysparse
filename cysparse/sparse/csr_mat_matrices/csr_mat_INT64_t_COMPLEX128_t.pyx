...
from cysparse.sparse.ll_mat_matrices.ll_mat_INT64_t_COMPLEX128_t cimport LLSparseMatrix_INT64_t_COMPLEX128_t, MakeLLSparseMatrix_INT64_t_COMPLEX128_t
...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT64_t_COMPLEX128_t cimport csr_to_csc_kernel_INT64_t_COMPLEX128_t, csc_to_csr_kernel_INT64_t_COMPLEX128_t, csr_to_ll_kernel_INT64_t_COMPLEX128_t
...
include "csr_mat_helpers/csr_mat_is_symmetric_INT64_t_COMPLEX128_t.pxi"
...
cdef class CSRSparseMatrix_INT64_t_COMPLEX128_t(ImmutableSparseMatrix_INT64_t_COMPLEX128_t):
    ...
    def copy(self):
        ...
        self_copy = CSRSparseMatrix_INT64_t_COMPLEX128_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def to_ll(self):
        ...
        csr_to_ll_kernel_INT64_t_COMPLEX128_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.col, <COMPLEX128_t *>self.val,
                       root, col, link, val)

        return MakeLLSparseMatrix_INT64_t_COMPLEX128_t(nrow=self.__nrow,
                                                 ncol=self.__ncol,
                                                 nnz=self.__nnz,
                                                 free=free,
                                                 nalloc=nalloc,
    ...
    def matdot(self, B):
        ...
        # CASES
        if isinstance(B, CSCSparseMatrix_INT64_t_COMPLEX128_t):
            return multiply_csr_mat_by_csc_mat_INT64_t_COMPLEX128_t(self, B)
        else:
...
# Factory methods
########################################################################################################################
cdef MakeCSRSparseMatrix_INT64_t_COMPLEX128_t(INT64_t nrow,
                                        INT64_t ncol,
                                        INT64_t nnz,
                                        INT64_t * ind,
                                        INT64_t * col,
    ...
    """
    cdef CSRSparseMatrix_INT64_t_COMPLEX128_t csr_mat

    csr_mat = CSRSparseMatrix_INT64_t_COMPLEX128_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, store_symmetric=store_symmetric, store_zero=store_zero)
