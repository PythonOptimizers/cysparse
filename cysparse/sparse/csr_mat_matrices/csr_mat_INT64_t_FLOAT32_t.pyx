...
cdef class CSRSparseMatrix_INT64_t_FLOAT32_t(ImmutableSparseMatrix_INT64_t_FLOAT32_t):
    ...
    def matvec_htransp(self, b):
        ...
        return multiply_conjugate_transposed_csr_mat_with_numpy_vector_INT64_t_FLOAT32_t(self, b)
    ...
    def matvec_conj(self, b):
        ...
        """
        assert are_mixed_types_compatible(FLOAT32_T, b.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(FLOAT32_T)
        return multiply_conjugated_csr_mat_with_numpy_vector_INT64_t_FLOAT32_t(self, b)
...
# Factory methods
########################################################################################################################
cdef MakeCSRSparseMatrix_INT64_t_FLOAT32_t(INT64_t nrow, INT64_t ncol, INT64_t nnz, INT64_t * ind, INT64_t * col, FLOAT32_t * val, bint __is_symmetric):
    """
    ...
    csr_mat = CSRSparseMatrix_INT64_t_FLOAT32_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, __is_symmetric=__is_symmetric)
