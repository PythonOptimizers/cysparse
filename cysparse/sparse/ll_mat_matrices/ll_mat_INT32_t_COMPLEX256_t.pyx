...
from cysparse.sparse.ll_mat_views.ll_mat_view_INT32_t_COMPLEX256_t cimport PyLLSparseMatrixView_Check
...
from cysparse.sparse.csr_mat_matrices.csr_mat_INT32_t_COMPLEX256_t cimport MakeCSRSparseMatrix_INT32_t_COMPLEX256_t
from cysparse.sparse.csc_mat_matrices.csc_mat_INT32_t_COMPLEX256_t cimport MakeCSCSparseMatrix_INT32_t_COMPLEX256_t
...
include "ll_mat_kernel/ll_mat_assignment_kernel_INT32_t_COMPLEX256_t.pxi"
...
########################################################################################################################
from cysparse.sparse.ll_mat_views.ll_mat_view_INT32_t_COMPLEX256_t cimport LLSparseMatrixView_INT32_t_COMPLEX256_t, MakeLLSparseMatrixView_INT32_t_COMPLEX256_t
...
cdef class LLSparseMatrix_INT32_t_COMPLEX256_t(MutableSparseMatrix_INT32_t_COMPLEX256_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT32_t_COMPLEX256_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=self.__use_symmetric_storage)
    ...
    def create_transpose(self):
        ...
        cdef:
            INT32_t i, j, k
            LLSparseMatrix_INT32_t_COMPLEX256_t transpose

        if self.__use_symmetric_storage:
            return self.copy()
        else:
            transpose = LLSparseMatrix_INT32_t_COMPLEX256_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=self.__use_symmetric_storage)
    ...
    def create_conjugate_transpose(self):
        ...
        cdef:
            INT32_t i, j, k
            LLSparseMatrix_INT32_t_COMPLEX256_t conjugate_transpose

        if self.__use_symmetric_storage:
            return self.create_conjugate()

        else:
            conjugate_transpose = LLSparseMatrix_INT32_t_COMPLEX256_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=self.__use_symmetric_storage)
    ...
    def create_conjugate(self):
        cdef:
            INT32_t i, j, k
            LLSparseMatrix_INT32_t_COMPLEX256_t conjugate

        conjugate = <LLSparseMatrix_INT32_t_COMPLEX256_t> self.copy()
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT32_t_COMPLEX256_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind, col=col,
                                                     val=val,
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT32_t_COMPLEX256_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind,
                                                     row=row,
    ...
    cdef assign(self, LLSparseMatrixView_INT32_t_COMPLEX256_t view, object obj):
        ...
        Args:
            view: An ``LLSparseMatrixView_INT32_t_COMPLEX256_t`` that points to this matrix (``self``).
            obj: Can be a:
                - ``LLSparseMatrix``;
                - ``LLSparseMatrixView``;
        ...
        cdef:
            ...
            LLSparseMatrixView_INT32_t_COMPLEX256_t A_view
        ...
        if self.__use_symmetric_storage:
            if PySparseMatrix_Check(obj):
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A = <LLSparseMatrix_INT32_t_COMPLEX256_t> obj
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A_view = <LLSparseMatrixView_INT32_t_COMPLEX256_t> obj
    ...
    def __setitem__(self, tuple key, value):
        ...
        cdef LLSparseMatrixView_INT32_t_COMPLEX256_t view
        ...
        if not PyInt_Check(<PyObject *>key[0]) or not PyInt_Check(<PyObject *>key[1]):
            ...
            view = MakeLLSparseMatrixView_INT32_t_COMPLEX256_t(self, <PyObject *>key[0], <PyObject *>key[1])
    ...
    def tril(self, int k = 0):
        ...
        cdef:
            INT32_t i, j, k_
            LLSparseMatrix_INT32_t_COMPLEX256_t ll_mat_tril

        ll_mat_tril = LLSparseMatrix_INT32_t_COMPLEX256_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=False)
    ...
    def triu(self, int k = 0):
        ...
        cdef:
            INT32_t i, j, k_
            LLSparseMatrix_INT32_t_COMPLEX256_t ll_mat_triu

        ll_mat_triu = LLSparseMatrix_INT32_t_COMPLEX256_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=False)
    ...
    # Addition
    ####################################################################################################################
    def shift(self, sigma, LLSparseMatrix_INT32_t_COMPLEX256_t B):
        ...
        if self.__use_symmetric_storage == B.__use_symmetric_storage:
            ...
            for i from 0 <= i < B.__nrow:
                ...
                while k != -1:
                    update_ll_mat_item_add_INT32_t_COMPLEX256_t(self, i, B.col[k], casted_sigma * B.val[k])
                    k = B.link[k]
            ...
            for i from 0 <= i < B.__nrow:
                ...
                while k != -1:
                    ...
                    update_ll_mat_item_add_INT32_t_COMPLEX256_t(self, i, j, v)
                    if i != j:
                        update_ll_mat_item_add_INT32_t_COMPLEX256_t(self, j, i, v)
                    k = B.link[k]
        else:
    ...
    def update_add_at(self, id1, id2, val):
        ...
        See :meth:`update_add_at_with_numpy_arrays_INT32_t_COMPLEX256_t`.
        """
        return update_add_at_with_numpy_arrays_INT32_t_COMPLEX256_t(self, id1, id2, val)
    ...
    def matdot_transp(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            ...
            return multiply_transposed_ll_mat_by_ll_mat_INT32_t_COMPLEX256_t(self, B)
        elif cnp.PyArray_Check(B):
            assert are_mixed_types_compatible(COMPLEX256_T, B.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(COMPLEX256_T)
