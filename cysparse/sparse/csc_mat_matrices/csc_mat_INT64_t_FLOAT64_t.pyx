...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT64_t_FLOAT64_t cimport csr_to_csc_kernel_INT64_t_FLOAT64_t
...
cdef class CSCSparseMatrix_INT64_t_FLOAT64_t(ImmutableSparseMatrix_INT64_t_FLOAT64_t):
    ...
    def triu(self, int k):
        ...
        cdef FLOAT64_t * csr_val = <FLOAT64_t *> PyMem_Malloc(self.__nnz * sizeof(FLOAT64_t))
        if not csr_val:
            PyMem_Free(ind)
            PyMem_Free(row)
            PyMem_Free(val)
