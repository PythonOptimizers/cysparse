...
cdef bint update_ll_mat_item_add_INT64_t_FLOAT128_t(LLSparseMatrix_INT64_t_FLOAT128_t A, INT64_t i, INT64_t j, FLOAT128_t x):
    """
    Update-add matrix entry: ``A[i,j] += x``

    Args:
        A: :class:`LLSparseMatrix_INT64_t_FLOAT128_t` to update.
        i, j: Coordinates of item to update.
        x (FLOAT128_t): Value to add to element ``(i, j)`` to update.
