...
class CySparseCSRConjugatedMultiplicationWithANumpyVectorTestCase(CySparseCSRConjugatedMultiplicationWithANumpyVectorBaseTestCase):
    ...
    def test_simple_multiplication_one_by_one(self):
        ...
        csr_y = self.l_1_1_csr.matvec_conj(self.x_COMPLEX64_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_2.matvec_conj(self.x_COMPLEX128_t)
        csr_y = self.l_1_2_csr.matvec_conj(self.x_COMPLEX128_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_3.matvec_conj(self.x_COMPLEX256_t)
        csr_y = self.l_1_3_csr.matvec_conj(self.x_COMPLEX256_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
        ...
        csr_y = self.l_2_1_csr.matvec_conj(self.x_COMPLEX64_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_2.matvec_conj(self.x_COMPLEX128_t)
        csr_y = self.l_2_2_csr.matvec_conj(self.x_COMPLEX128_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_3.matvec_conj(self.x_COMPLEX256_t)
        csr_y = self.l_2_3_csr.matvec_conj(self.x_COMPLEX256_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
...
class CySparseCSRConjugatedMultiplicationWithAStridedNumpyVectorTestCase(CySparseCSRConjugatedMultiplicationWithANumpyVectorBaseTestCase):
    ...
    def test_simple_multiplication_one_by_one(self):
        ...
        l_y = self.l_1_1.matvec_conj(self.x_COMPLEX64_t)
        csr_y = self.l_1_1_csr.matvec_conj(self.x_strided_COMPLEX64_t[::self.stride_factor])
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_2.matvec_conj(self.x_COMPLEX128_t)
        csr_y = self.l_1_2_csr.matvec_conj(self.x_strided_COMPLEX128_t[::self.stride_factor])
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_1_3.matvec_conj(self.x_COMPLEX256_t)
        csr_y = self.l_1_3_csr.matvec_conj(self.x_strided_COMPLEX256_t[::self.stride_factor])
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
        ...
        l_y = self.l_2_1.matvec_conj(self.x_COMPLEX64_t)
        csr_y = self.l_2_1_csr.matvec_conj(self.x_strided_COMPLEX64_t[::self.stride_factor])
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_2.matvec_conj(self.x_COMPLEX128_t)
        csr_y = self.l_2_2_csr.matvec_conj(self.x_strided_COMPLEX128_t[::self.stride_factor])
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
  
        l_y = self.l_2_3.matvec_conj(self.x_COMPLEX256_t)
        csr_y = self.l_2_3_csr.matvec_conj(self.x_strided_COMPLEX256_t[::self.stride_factor])
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csr_y[i])
