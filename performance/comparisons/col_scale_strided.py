"""
This file compares different implementations of col_scale, i.e. scale the i:sup:`th` column of A by ``v[i]`` in place for ``i=0, ..., ncol-1``.
...
class LLMatColScaleStridedBenchmark(benchmark.Benchmark):
    ...
    def test_pysparse(self):
        self.A_p.col_scale(self.v[::self.stride])
        return

    def test_cysparse(self):
        self.A_c.col_scale(self.v[::self.stride])
        return


class LLMatColScaleStridedBenchmark_1(LLMatColScaleStridedBenchmark):


    label = "Simple col_scale with 1000 elements and size = 10,000 and a strided NumPy vector (stride = 10)"
    each = 10
...
class LLMatColScaleStridedBenchmark_2(LLMatColScaleStridedBenchmark):


    label = "Simple col_scale with 10,000 elements and size = 100,000 and a strided NumPy vector (stride = 10)"
    each = 10
...
class LLMatColScaleStridedBenchmark_3(LLMatColScaleStridedBenchmark):


    label = "Simple col_scale with 80,000 elements and size = 100,000 and a strided NumPy vector (stride = 10)"
    each = 10
...
class LLMatColScaleStridedBenchmark_4(LLMatColScaleStridedBenchmark):


    label = "Simple col_scale with 10,000 elements and size = 100,000 and a strided NumPy vector (stride = 739)"
    each = 10
