...
cdef class CSCSparseMatrix_INT64_t_INT64_t(ImmutableSparseMatrix_INT64_t_INT64_t):
    ...
    def to_csr(self):
        ...
        return MakeCSRSparseMatrix_INT64_t_INT64_t(self.__nrow, self.__ncol, self.__nnz, ind, col, val, is_symmetric=self.is_symmetric, store_zeros=self.store_zeros)
...
# Factory methods
########################################################################################################################
cdef MakeCSCSparseMatrix_INT64_t_INT64_t(INT64_t nrow, INT64_t ncol, INT64_t nnz, INT64_t * ind, INT64_t * row, INT64_t * val, bint is_symmetric, bint store_zeros):
    """
    ...
    """
    csc_mat = CSCSparseMatrix_INT64_t_INT64_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, is_symmetric=is_symmetric, store_zeros=store_zeros)
