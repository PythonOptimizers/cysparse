...
cdef cnp.ndarray[cnp.npy_int64, ndim=1, mode='c'] multiply_csc_mat_with_numpy_vector_INT32_t_INT64_t(CSCSparseMatrix_INT32_t_INT64_t A, cnp.ndarray[cnp.npy_int64, ndim=1] b):
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__store_symmetric:
            pass
            multiply_sym_csc_mat_with_numpy_vector_kernel_INT32_t_INT64_t(A_nrow, A_ncol, b_data, c_data, A.val, A.row, A.ind, 1)
        else:
            multiply_csc_mat_with_numpy_vector_kernel_INT32_t_INT64_t(A_nrow, A_ncol, b_data, c_data, A.val, A.row, A.ind, 1)
    else:
        if A.__store_symmetric:
