...
# A * b
######################
cdef cnp.ndarray[cnp.npy_float64, ndim=1, mode='c'] multiply_csc_mat_with_numpy_vector_INT64_t_FLOAT64_t(CSCSparseMatrix_INT64_t_FLOAT64_t A, cnp.ndarray[cnp.npy_float64, ndim=1] b):
    """
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__store_symmetric:
            pass
            multiply_sym_csc_mat_with_numpy_vector_kernel_INT64_t_FLOAT64_t(A_nrow, A_ncol, b_data, c_data, A.val, A.row, A.ind, 1)
        else:
            multiply_csc_mat_with_numpy_vector_kernel_INT64_t_FLOAT64_t(A_nrow, A_ncol, b_data, c_data, A.val, A.row, A.ind, 1)
    else:
        if A.__store_symmetric:
            multiply_sym_csc_mat_with_strided_numpy_vector_kernel_INT64_t_FLOAT64_t(A.nrow, A.ncol,
                                                                 b_data, b.strides[0] / sd,
                                                                 c_data, c.strides[0] / sd,
                                                                 A.val, A.row, A.ind)
        else:
            multiply_csc_mat_with_strided_numpy_vector_kernel_INT64_t_FLOAT64_t(A.nrow, A.ncol,
                                                             b_data, b.strides[0] / sd,
                                                             c_data, c.strides[0] / sd,
                                                             A.val, A.row, A.ind)
...
# A^t * b
######################
cdef cnp.ndarray[cnp.npy_float64, ndim=1, mode='c'] multiply_transposed_csc_mat_with_numpy_vector_INT64_t_FLOAT64_t(CSCSparseMatrix_INT64_t_FLOAT64_t A, cnp.ndarray[cnp.npy_float64, ndim=1] b):
    """
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__store_symmetric:
            multiply_sym_csc_mat_with_numpy_vector_kernel_INT64_t_FLOAT64_t(A_nrow, A_ncol, b_data, c_data, A.val, A.row, A.ind)
        else:
            multiply_tranposed_csc_mat_with_numpy_vector_kernel_INT64_t_FLOAT64_t(A_nrow, A_ncol, b_data, c_data,
         A.val, A.row, A.ind)
    else:
        if A.__store_symmetric:
            multiply_sym_csc_mat_with_strided_numpy_vector_kernel_INT64_t_FLOAT64_t(A.nrow, A_ncol,
                                                                 b_data, b.strides[0] / sd,
                                                                 c_data, c.strides[0] / sd,
                                                                 A.val, A.row, A.ind)
        else:
            multiply_tranposed_csc_mat_with_strided_numpy_vector_kernel_INT64_t_FLOAT64_t(A_nrow, A_ncol,
                                                                                      b_data, b.strides[0] / sd,
                                                                                      c_data, c.strides[0] / sd,
                                                                                      A.val, A.row, A.ind)
