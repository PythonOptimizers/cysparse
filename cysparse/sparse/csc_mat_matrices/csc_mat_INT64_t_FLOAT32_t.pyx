...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT64_t_FLOAT32_t cimport csr_to_csc_kernel_INT64_t_FLOAT32_t
...
cdef class CSCSparseMatrix_INT64_t_FLOAT32_t(ImmutableSparseMatrix_INT64_t_FLOAT32_t):
    ...
    def triu(self, int k):
        ...
        cdef INT64_t * csr_ind = <INT64_t *> PyMem_Malloc((self.__nrow + 1) * sizeof(INT64_t))
        if not csr_ind:
            PyMem_Free(ind)
            PyMem_Free(row)
            PyMem_Free(val)
        ...
        cdef INT64_t * csr_col = <INT64_t *> PyMem_Malloc(self.__nnz * sizeof(INT64_t))
        if not csr_col:
            PyMem_Free(ind)
            PyMem_Free(row)
            PyMem_Free(val)
        ...
        cdef FLOAT32_t * csr_val = <FLOAT32_t *> PyMem_Malloc(self.__nnz * sizeof(FLOAT32_t))
        if not csr_val:
            PyMem_Free(ind)
            PyMem_Free(row)
            PyMem_Free(val)
