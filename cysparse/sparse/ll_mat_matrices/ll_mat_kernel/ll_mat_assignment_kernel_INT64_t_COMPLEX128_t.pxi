...
cdef bint update_ll_mat_item_add_INT64_t_COMPLEX128_t(LLSparseMatrix_INT64_t_COMPLEX128_t A, INT64_t i, INT64_t j, COMPLEX128_t x):
    """
    Update-add matrix entry: ``A[i,j] += x``

    Args:
        A: :class:`LLSparseMatrix_INT64_t_COMPLEX128_t` to update.
        i, j: Coordinates of item to update.
        x (COMPLEX128_t): Value to add to element ``(i, j)`` to update.
    ...
    if not A.__use_zero_storage and x == 0.0:
        return True
    ...
    if col == j:
        ...
        if A.__use_zero_storage and x == 0.0:
