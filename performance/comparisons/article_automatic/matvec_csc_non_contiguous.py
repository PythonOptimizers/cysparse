"""
This file compares different implementations of ``matvec``, i.e. :math:`A * x` when :math:`x` is non-contiguous.
...
class LLMatMatVecBenchmark(benchmark.Benchmark):
    ...
    def test_cysparse(self):
        self.w_c = self.CSC_c * self.v[::self.non_contiguity]
        return

    def test_cysparse2(self):
        self.CSC_c.matvec(self.v[::self.non_contiguity])
        return

    def test_scipy_sparse(self):
        self.w_s = self.CSC_s * self.v[::self.non_contiguity]
        return

    def test_scipy_sparse2(self):
        self.CSC_s._mul_vector(self.v[::self.non_contiguity])
