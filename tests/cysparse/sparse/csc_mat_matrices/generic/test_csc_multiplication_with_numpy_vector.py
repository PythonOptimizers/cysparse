...
"""
This file tests the multiplication of an :class:`CSCSparseMatrix` matricix with a :program:`NumPy` vector.
...
class CySparseCSCMultiplicationWithANumpyVectorTestCase(CySparseCSCMultiplicationWithANumpyVectorBaseTestCase):
    ...
    def test_simple_multiplication_one_by_one(self):
        ...
        csc_y = self.l_1_1_csc.matvec(self.x_INT32_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_1_2.matvec(self.x_INT64_t)
        csc_y = self.l_1_2_csc.matvec(self.x_INT64_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_1_3.matvec(self.x_FLOAT32_t)
        csc_y = self.l_1_3_csc.matvec(self.x_FLOAT32_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_1_4.matvec(self.x_FLOAT64_t)
        csc_y = self.l_1_4_csc.matvec(self.x_FLOAT64_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_1_5.matvec(self.x_FLOAT128_t)
        csc_y = self.l_1_5_csc.matvec(self.x_FLOAT128_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_1_6.matvec(self.x_COMPLEX64_t)
        csc_y = self.l_1_6_csc.matvec(self.x_COMPLEX64_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_1_7.matvec(self.x_COMPLEX128_t)
        csc_y = self.l_1_7_csc.matvec(self.x_COMPLEX128_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_1_8.matvec(self.x_COMPLEX256_t)
        csc_y = self.l_1_8_csc.matvec(self.x_COMPLEX256_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
        ...
        csc_y = self.l_2_1_csc.matvec(self.x_INT32_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_2_2.matvec(self.x_INT64_t)
        csc_y = self.l_2_2_csc.matvec(self.x_INT64_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_2_3.matvec(self.x_FLOAT32_t)
        csc_y = self.l_2_3_csc.matvec(self.x_FLOAT32_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_2_4.matvec(self.x_FLOAT64_t)
        csc_y = self.l_2_4_csc.matvec(self.x_FLOAT64_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_2_5.matvec(self.x_FLOAT128_t)
        csc_y = self.l_2_5_csc.matvec(self.x_FLOAT128_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_2_6.matvec(self.x_COMPLEX64_t)
        csc_y = self.l_2_6_csc.matvec(self.x_COMPLEX64_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_2_7.matvec(self.x_COMPLEX128_t)
        csc_y = self.l_2_7_csc.matvec(self.x_COMPLEX128_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
  
        l_y = self.l_2_8.matvec(self.x_COMPLEX256_t)
        csc_y = self.l_2_8_csc.matvec(self.x_COMPLEX256_t)
        for i in xrange(self.nrow):
            self.failUnless(l_y[i] == csc_y[i])
...
class CySparseCSCMultiplicationWithAStridedNumpyVectorTestCase(CySparseCSCMultiplicationWithANumpyVectorBaseTestCase):
    ...
    def setUp(self):
        ...
        self.x_INT32_t = np.ones(self.ncol, dtype=np.int32)
        self.x_strided_INT32_t = np.empty(self.ncol * self.stride_factor, dtype=np.int32)
        ...
        self.x_INT64_t = np.ones(self.ncol, dtype=np.int64)
        self.x_strided_INT64_t = np.empty(self.ncol * self.stride_factor, dtype=np.int64)
        ...
        self.x_FLOAT32_t = np.ones(self.ncol, dtype=np.float32)
        self.x_strided_FLOAT32_t = np.empty(self.ncol * self.stride_factor, dtype=np.float32)
        ...
        self.x_FLOAT64_t = np.ones(self.ncol, dtype=np.float64)
        self.x_strided_FLOAT64_t = np.empty(self.ncol * self.stride_factor, dtype=np.float64)
        ...
        self.x_FLOAT128_t = np.ones(self.ncol, dtype=np.float128)
        self.x_strided_FLOAT128_t = np.empty(self.ncol * self.stride_factor, dtype=np.float128)
