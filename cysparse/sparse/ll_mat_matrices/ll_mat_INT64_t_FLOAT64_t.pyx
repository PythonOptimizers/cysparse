    ...
    cdef assign(self, LLSparseMatrixView_INT64_t_FLOAT64_t view, object obj):
        ...
        if self.__is_symmetric:
            if PySparseMatrix_Check(obj):
                ...
                for i from 0 <= i < nrow:
                    for j from 0 <= j <= i:
                        self.put(row_indices[i], col_indices[j], <FLOAT64_t> A.at(i, j))
                ...
                for i from 0 <= i < nrow:
                    for j from 0 <= j <= i:
                        self.put(row_indices[i], col_indices[j], <FLOAT64_t> A_view.at(i, j))
            ...
            if PySparseMatrix_Check(obj):
                ...
                for i from 0 <= i < nrow:
                    for j from 0 <= j < ncol:
                        self.put(row_indices[i], col_indices[j], <FLOAT64_t> A_view.safe_at(i, j))
