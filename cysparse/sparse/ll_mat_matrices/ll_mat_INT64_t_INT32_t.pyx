...
from cysparse.sparse.ll_mat_views.ll_mat_view_INT64_t_INT32_t cimport PyLLSparseMatrixView_Check
...
from cysparse.sparse.s_mat_matrices.s_mat_INT64_t_INT32_t cimport MutableSparseMatrix_INT64_t_INT32_t
from cysparse.sparse.ll_mat_matrices.ll_mat_INT64_t_INT32_t cimport LLSparseMatrix_INT64_t_INT32_t
from cysparse.sparse.ll_mat_views.ll_mat_view_INT64_t_INT32_t cimport LLSparseMatrixView_INT64_t_INT32_t

from cysparse.sparse.csr_mat_matrices.csr_mat_INT64_t_INT32_t cimport MakeCSRSparseMatrix_INT64_t_INT32_t
from cysparse.sparse.csc_mat_matrices.csc_mat_INT64_t_INT32_t cimport MakeCSCSparseMatrix_INT64_t_INT32_t
...
include "ll_mat_kernel/ll_mat_assignment_kernel_INT64_t_INT32_t.pxi"
...
########################################################################################################################
from cysparse.sparse.ll_mat_views.ll_mat_view_INT64_t_INT32_t cimport LLSparseMatrixView_INT64_t_INT32_t, MakeLLSparseMatrixView_INT64_t_INT32_t


########################################################################################################################
# CLASS LLSparseMatrix
########################################################################################################################
cdef class LLSparseMatrix_INT64_t_INT32_t(MutableSparseMatrix_INT64_t_INT32_t):
    """
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT64_t_INT32_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def create_transpose(self):
        ...
        cdef:
            INT64_t i, j, k
            LLSparseMatrix_INT64_t_INT32_t transpose

        if self.__store_symmetric:
            return self.copy()
        else:
            transpose = LLSparseMatrix_INT64_t_INT32_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT64_t_INT32_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind, col=col,
                                                     val=val,
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT64_t_INT32_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind,
                                                     row=row,
    ...
    ####################################################################################################################
    #                                            ### ASSIGN ###
    cdef assign(self, LLSparseMatrixView_INT64_t_INT32_t view, object obj):
        """
        Set ``A[..., ...] = obj`` directly.

        Args:
            view: An ``LLSparseMatrixView_INT64_t_INT32_t`` that points to this matrix (``self``).
            obj: Can be a:
                - ``LLSparseMatrix``;
                - ``LLSparseMatrixView``;
        ...
        cdef:
            ...
            LLSparseMatrixView_INT64_t_INT32_t A_view
        ...
        if self.__store_symmetric:
            if PySparseMatrix_Check(obj):
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A = <LLSparseMatrix_INT64_t_INT32_t> obj
                    ...
                    for i from 0 <= i < nrow:
                        for j from 0 <= j <= i:
                            self.put(row_indices[i], col_indices[j], <INT32_t> obj[i, j])
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A_view = <LLSparseMatrixView_INT64_t_INT32_t> obj
                ...
                for i from 0 <= i < nrow:
                    for j from 0 <= j <= i:
                        self.put(row_indices[i], col_indices[j], <INT32_t> obj)
            else:
    ...
    def __setitem__(self, tuple key, value):
        ...
        cdef LLSparseMatrixView_INT64_t_INT32_t view
        ...
        if not PyInt_Check(<PyObject *>key[0]) or not PyInt_Check(<PyObject *>key[1]):
            ...
            view = MakeLLSparseMatrixView_INT64_t_INT32_t(self, <PyObject *>key[0], <PyObject *>key[1])
    ...
    def put_triplet(self, id1, id2, b):
        ...
        ################################################################################################################
            ...
            ############################################################################################################
                ...
                # check if NumPy array type is compatible
                if not are_mixed_types_compatible(INT32_T, b.dtype):
                    raise TypeError('Value NumPy array must contain elements of the right value type (%s)' % cysparse_to_numpy_type(INT32_T))
    ...
    cpdef put_diagonal(self, INT64_t k, cnp.ndarray[cnp.npy_int32, ndim=1] b):
        """
    ...
    cdef fill_triplet(self, INT64_t * a_row, INT64_t * a_col, INT32_t * a_val):
        """
        Warning:
    ...
    def tril(self, int k = 0):
        ...
        ll_mat_tril = LLSparseMatrix_INT64_t_INT32_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    def triu(self, int k = 0):
        ...
        ll_mat_triu = LLSparseMatrix_INT64_t_INT32_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    # Addition
    ####################################################################################################################
    def shift(self, sigma, LLSparseMatrix_INT64_t_INT32_t B):
        ...
        cdef:
            INT32_t casted_sigma, v
            INT64_t k, i, j

        try:
            casted_sigma = <INT32_t> sigma
        except:
        ...
        if self.__store_symmetric == B.__store_symmetric:
            ...
            for i from 0 <= i < B.__nrow:
                ...
                while k != -1:
                    update_ll_mat_item_add_INT64_t_INT32_t(self, i, B.col[k], casted_sigma * B.val[k])
                    k = B.link[k]
            ...
            for i from 0 <= i < B.__nrow:
                ...
                while k != -1:
                    ...
                    update_ll_mat_item_add_INT64_t_INT32_t(self, i, j, v)
                    if i != j:
                        update_ll_mat_item_add_INT64_t_INT32_t(self, j, i, v)
                    k = B.link[k]
        else:
    ...
    def update_add_at(self, id1, id2, val):
        ...
        See :meth:`update_add_at_with_numpy_arrays_INT64_t_INT32_t`.
        """
        return update_add_at_with_numpy_arrays_INT64_t_INT32_t(self, id1, id2, val)
    ...
    def matvec(self, b):
        ...
        return multiply_ll_mat_with_numpy_vector_INT64_t_INT32_t(self, b)
    ...
    def matvec_transp(self, b):
        ...
        """
        assert are_mixed_types_compatible(INT32_T, b.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(INT32_T)
        return multiply_transposed_ll_mat_with_numpy_vector_INT64_t_INT32_t(self, b)
    ...
    def matdot(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            return multiply_two_ll_mat_INT64_t_INT32_t(self, B)
    ...
    def matdot_transp(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            ...
            return multiply_transposed_ll_mat_by_ll_mat_INT64_t_INT32_t(self, B)
        elif cnp.PyArray_Check(B):
            assert are_mixed_types_compatible(INT32_T, B.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(INT32_T)
            return multiply_transposed_ll_mat_with_numpy_ndarray_INT32_t(self, B)
        else:
            raise NotImplementedError("Multiplication with this kind of object not allowed")
    ...
    def col_scale(self, cnp.ndarray[cnp.npy_int32, ndim=1] v):
        """
        ...
        cdef INT32_t * v_data = <INT32_t *> cnp.PyArray_DATA(v)
    ...
    def row_scale(self, cnp.ndarray[cnp.npy_int32, ndim=1] v):
        """
...
# Factory methods
########################################################################################################################
cdef MakeLLSparseMatrix_INT64_t_INT32_t(INT64_t nrow,
                                        INT64_t ncol,
                                        INT64_t nnz,
                                        INT64_t free,
                                        INT64_t nalloc,
    ...
    """
    cdef LLSparseMatrix_INT64_t_INT32_t ll_mat

    ll_mat = LLSparseMatrix_INT64_t_INT32_t(control_object=unexposed_value,
                                           no_memory=True,
                                           size_hint=nalloc,
                                           nrow=nrow,
                                           ncol=ncol,
