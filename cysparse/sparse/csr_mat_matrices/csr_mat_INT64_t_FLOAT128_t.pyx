...
from cysparse.sparse.ll_mat_matrices.ll_mat_INT64_t_FLOAT128_t cimport LLSparseMatrix_INT64_t_FLOAT128_t, MakeLLSparseMatrix_INT64_t_FLOAT128_t
from cysparse.sparse.csc_mat_matrices.csc_mat_INT64_t_FLOAT128_t cimport CSCSparseMatrix_INT64_t_FLOAT128_t, MakeCSCSparseMatrix_INT64_t_FLOAT128_t
...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT64_t_FLOAT128_t cimport csr_to_csc_kernel_INT64_t_FLOAT128_t, csc_to_csr_kernel_INT64_t_FLOAT128_t, csr_to_ll_kernel_INT64_t_FLOAT128_t
...
include "csr_mat_kernel/csr_mat_multiplication_by_numpy_vector_kernel_INT64_t_FLOAT128_t.pxi"
include "csr_mat_helpers/csr_mat_multiplication_INT64_t_FLOAT128_t.pxi"
include "csr_mat_helpers/csr_mat_is_symmetric_INT64_t_FLOAT128_t.pxi"
...
cdef class CSRSparseMatrix_INT64_t_FLOAT128_t(ImmutableSparseMatrix_INT64_t_FLOAT128_t):
    ...
    def copy(self):
        ...
        cdef CSRSparseMatrix_INT64_t_FLOAT128_t self_copy
        ...
        self_copy = CSRSparseMatrix_INT64_t_FLOAT128_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)

        val = <FLOAT128_t *> PyMem_Malloc(nnz * sizeof(FLOAT128_t))
        if not val:
            raise MemoryError()
        memcpy(val, self.val, nnz * sizeof(FLOAT128_t))
        self_copy.val = val
    ...
    def memory_real_in_bytes(self):
        ...
        # val
        total_memory += self.__nnz * sizeof(FLOAT128_t)
    ...
    def tril(self, int k = 0):
        ...
        return MakeCSRSparseMatrix_INT64_t_FLOAT128_t(self.__nrow,
                                                  self.__ncol,
                                                  nnz,
                                                  ind,
                                                  col,
    ...
    def triu(self, int k = 0):
        ...
        if self.__store_symmetric:
            ...
            csc_val = <FLOAT128_t *> PyMem_Malloc(self.__nnz * sizeof(FLOAT128_t))
            if not csc_val:
                PyMem_Free(ind)
                PyMem_Free(col)
                PyMem_Free(val)
            ...
            csc_to_csr_kernel_INT64_t_FLOAT128_t(self.__nrow, self.__ncol, nnz,
    ...
    def to_csc(self):
        ...
        csr_to_csc_kernel_INT64_t_FLOAT128_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.col, <FLOAT128_t *>self.val,
                       ind, row, val)

        return MakeCSCSparseMatrix_INT64_t_FLOAT128_t(self.__nrow,
                                                  self.__ncol,
                                                  self.__nnz,
                                                  ind,
                                                  row,
    ...
    def to_ll(self):
        ...
        csr_to_ll_kernel_INT64_t_FLOAT128_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT64_t *>self.ind, <INT64_t *>self.col, <FLOAT128_t *>self.val,
                       root, col, link, val)

        return MakeLLSparseMatrix_INT64_t_FLOAT128_t(nrow=self.__nrow,
                                                 ncol=self.__ncol,
                                                 nnz=self.__nnz,
                                                 free=free,
                                                 nalloc=nalloc,
    ...
    def matdot(self, B):
        ...
        # CASES
        if isinstance(B, CSCSparseMatrix_INT64_t_FLOAT128_t):
            return multiply_csr_mat_by_csc_mat_INT64_t_FLOAT128_t(self, B)
        else:
...
# Factory methods
########################################################################################################################
cdef MakeCSRSparseMatrix_INT64_t_FLOAT128_t(INT64_t nrow,
                                        INT64_t ncol,
                                        INT64_t nnz,
                                        INT64_t * ind,
                                        INT64_t * col,
    ...
    csr_mat = CSRSparseMatrix_INT64_t_FLOAT128_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, store_symmetric=store_symmetric, store_zero=store_zero)
