...
# A * b
######################
cdef cnp.ndarray[cnp.npy_complex256, ndim=1, mode='c'] multiply_csr_mat_with_numpy_vector_INT64_t_COMPLEX256_t(CSRSparseMatrix_INT64_t_COMPLEX256_t A, cnp.ndarray[cnp.npy_complex256, ndim=1] b):
    """
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__use_symmetric_storage:
            ...
            multiply_csr_mat_with_numpy_vector_kernel_INT64_t_COMPLEX256_t(A_nrow, b_data, c_data, A.val, A.col, A.ind)
    else:
        if A.__use_symmetric_storage:
            ...
            multiply_csr_mat_with_strided_numpy_vector_kernel_INT64_t_COMPLEX256_t(A.nrow,
                                                             b_data, b.strides[0] / sd,
                                                             c_data, c.strides[0] / sd,
                                                             A.val, A.col, A.ind)
...
# A^t * b
######################
cdef cnp.ndarray[cnp.npy_complex256, ndim=1, mode='c'] multiply_transposed_csr_mat_with_numpy_vector_INT64_t_COMPLEX256_t(CSRSparseMatrix_INT64_t_COMPLEX256_t A, cnp.ndarray[cnp.npy_complex256, ndim=1] b):
    """
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__use_symmetric_storage:
            ...
            multiply_tranposed_csr_mat_with_numpy_vector_kernel_INT64_t_COMPLEX256_t(A_nrow, A_ncol, b_data, c_data,
         A.val, A.col, A.ind)
    else:
        if A.__use_symmetric_storage:
            multiply_sym_csr_mat_with_strided_numpy_vector_kernel_INT64_t_COMPLEX256_t(A.nrow,
            ...
            multiply_tranposed_csr_mat_with_strided_numpy_vector_kernel_INT64_t_COMPLEX256_t(A_nrow, A_ncol,
                                                                                      b_data, b.strides[0] / sd,
                                                                                      c_data, c.strides[0] / sd,
                                                                                      A.val, A.col, A.ind)
...
# A^h * b
######################
cdef cnp.ndarray[cnp.npy_complex256, ndim=1, mode='c'] multiply_conjugate_transposed_csr_mat_with_numpy_vector_INT64_t_COMPLEX256_t(CSRSparseMatrix_INT64_t_COMPLEX256_t A, cnp.ndarray[cnp.npy_complex256, ndim=1] b):
    """
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__use_symmetric_storage:
            multiply_conjugate_transposed_sym_csr_mat_with_numpy_vector_kernel_INT64_t_COMPLEX256_t(A_nrow, A_ncol, b_data, c_data, A.val, A.col, A.ind)
        else:
            multiply_conjugate_transposed_csr_mat_with_numpy_vector_kernel_INT64_t_COMPLEX256_t(A_nrow, A_ncol, b_data, c_data,
         A.val, A.col, A.ind)
    else:
        if A.__use_symmetric_storage:
            multiply_conjugate_transposed_sym_csr_mat_with_strided_numpy_vector_kernel_INT64_t_COMPLEX256_t(A.nrow, A.ncol,
                                                                 b_data, b.strides[0] / sd,
                                                                 c_data, c.strides[0] / sd,
                                                                 A.val, A.col, A.ind)
        else:
            multiply_conjugate_tranposed_csr_mat_with_strided_numpy_vector_kernel_INT64_t_COMPLEX256_t(A_nrow, A_ncol,
                                                                                      b_data, b.strides[0] / sd,
                                                                                      c_data, c.strides[0] / sd,
                                                                                      A.val, A.col, A.ind)
...
# conj(A) * b
######################
cdef cnp.ndarray[cnp.npy_complex256, ndim=1, mode='c'] multiply_conjugated_csr_mat_with_numpy_vector_INT64_t_COMPLEX256_t(CSRSparseMatrix_INT64_t_COMPLEX256_t A, cnp.ndarray[cnp.npy_complex256, ndim=1] b):
    """
    ...
    if cnp.PyArray_ISCONTIGUOUS(b):
        if A.__use_symmetric_storage:
            multiply_conjugated_sym_csr_mat_with_numpy_vector_kernel_INT64_t_COMPLEX256_t(A_nrow, b_data, c_data, A.val, A.col, A.ind)
        else:
            multiply_conjugated_csr_mat_with_numpy_vector_kernel_INT64_t_COMPLEX256_t(A_nrow, b_data, c_data, A.val, A.col, A.ind)

    else:
        if A.__use_symmetric_storage:
            multiply_conjugated_sym_csr_mat_with_strided_numpy_vector_kernel_INT64_t_COMPLEX256_t(A.nrow,
                                                                 b_data, b.strides[0] / sd,
                                                                 c_data, c.strides[0] / sd,
                                                                 A.val, A.col, A.ind)
        else:
            multiply_conjugated_csr_mat_with_strided_numpy_vector_kernel_INT64_t_COMPLEX256_t(A.nrow,
                                                             b_data, b.strides[0] / sd,
                                                             c_data, c.strides[0] / sd,
                                                             A.val, A.col, A.ind)
