...
cdef class LLSparseMatrix_INT64_t_COMPLEX256_t(MutableSparseMatrix_INT64_t_COMPLEX256_t):
    ...
    def create_transpose(self):
        ...
        if self.is_symmetric:
            ...
            transpose = LLSparseMatrix_INT64_t_COMPLEX256_t(control_object=unexposed_value, nrow=self.ncol, ncol=self.nrow, size_hint=self.nnz, store_zeros=self.store_zeros, is_symmetric=self.is_symmetric)
    ...
    def create_conjugate_transpose(self):
        ...
        if self.is_symmetric:
            ...
            conjugate_transpose = LLSparseMatrix_INT64_t_COMPLEX256_t(control_object=unexposed_value, nrow=self.ncol, ncol=self.nrow, size_hint=self.nnz, store_zeros=self.store_zeros, is_symmetric=self.is_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT64_t_COMPLEX256_t(nrow=self.nrow, ncol=self.ncol, nnz=self.nnz, ind=ind, col=col, val=val, is_symmetric=self.is_symmetric)
