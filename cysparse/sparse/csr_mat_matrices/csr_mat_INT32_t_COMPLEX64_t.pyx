...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT32_t_COMPLEX64_t cimport csr_to_csc_kernel_INT32_t_COMPLEX64_t, csc_to_csr_kernel_INT32_t_COMPLEX64_t, csr_to_ll_kernel_INT32_t_COMPLEX64_t
...
include "csr_mat_helpers/csr_mat_is_symmetric_INT32_t_COMPLEX64_t.pxi"
...
cdef class CSRSparseMatrix_INT32_t_COMPLEX64_t(ImmutableSparseMatrix_INT32_t_COMPLEX64_t):
    ...
    def copy(self):
        ...
        self_copy = CSRSparseMatrix_INT32_t_COMPLEX64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def to_ll(self):
        ...
        csr_to_ll_kernel_INT32_t_COMPLEX64_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT32_t *>self.ind, <INT32_t *>self.col, <COMPLEX64_t *>self.val,
                       root, col, link, val)
    ...
    def matdot(self, B):
        ...
        # CASES
        if isinstance(B, CSCSparseMatrix_INT32_t_COMPLEX64_t):
            return multiply_csr_mat_by_csc_mat_INT32_t_COMPLEX64_t(self, B)
        else:
...
cdef MakeCSRSparseMatrix_INT32_t_COMPLEX64_t(INT32_t nrow,
    ...
    csr_mat = CSRSparseMatrix_INT32_t_COMPLEX64_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, store_symmetric=store_symmetric, store_zero=store_zero)
