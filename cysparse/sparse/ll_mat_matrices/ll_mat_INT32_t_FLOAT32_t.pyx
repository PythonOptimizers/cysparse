...
cdef class LLSparseMatrix_INT32_t_FLOAT32_t(MutableSparseMatrix_INT32_t_FLOAT32_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT32_t_FLOAT32_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, store_zeros=self.__store_zeros, is_symmetric=self.__is_symmetric)
    ...
    def create_transpose(self):
        ...
        if self.__is_symmetric:
            ...
            transpose = LLSparseMatrix_INT32_t_FLOAT32_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, store_zeros=self.__store_zeros, is_symmetric=self.__is_symmetric)
    ...
    cdef assign(self, LLSparseMatrixView_INT32_t_FLOAT32_t view, object obj):
        ...
        if self.__is_symmetric:
            if PySparseMatrix_Check(obj):
                ...
                for i from 0 <= i < nrow:
                    for j from 0 <= j <= i:
                        self.put(row_indices[i], col_indices[j], <FLOAT32_t> A.at(i, j))
                ...
                for i from 0 <= i < nrow:
                    for j from 0 <= j <= i:
                        self.put(row_indices[i], col_indices[j], <FLOAT32_t> A_view.at(i, j))
            ...
            if PySparseMatrix_Check(obj):
                ...
                for i from 0 <= i < nrow:
                    for j from 0 <= j < ncol:
                        self.put(row_indices[i], col_indices[j], <FLOAT32_t> A_view.safe_at(i, j))
    ...
    def tril(self, int k):
        ...
        ll_mat_tril = LLSparseMatrix_INT32_t_FLOAT32_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zeros=self.__store_zeros, is_symmetric=False)
    ...
    def triu(self, int k):
        ...
        ll_mat_triu = LLSparseMatrix_INT32_t_FLOAT32_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zeros=self.__store_zeros, is_symmetric=False)
