...
from cysparse.sparse.s_mat_matrices.s_mat_INT32_t_COMPLEX256_t cimport ImmutableSparseMatrix_INT32_t_COMPLEX256_t, MutableSparseMatrix_INT32_t_COMPLEX256_t
from cysparse.sparse.ll_mat_matrices.ll_mat_INT32_t_COMPLEX256_t cimport LLSparseMatrix_INT32_t_COMPLEX256_t
from cysparse.sparse.csc_mat_matrices.csc_mat_INT32_t_COMPLEX256_t cimport CSCSparseMatrix_INT32_t_COMPLEX256_t, MakeCSCSparseMatrix_INT32_t_COMPLEX256_t
...
from cysparse.sparse.sparse_utils.generic.matrix_translations_INT32_t_COMPLEX256_t cimport csr_to_csc_kernel_INT32_t_COMPLEX256_t, csc_to_csr_kernel_INT32_t_COMPLEX256_t
...
include "csr_mat_kernel/csr_mat_multiplication_by_numpy_vector_kernel_INT32_t_COMPLEX256_t.pxi"
include "csr_mat_helpers/csr_mat_multiplication_INT32_t_COMPLEX256_t.pxi"
...
cdef class CSRSparseMatrix_INT32_t_COMPLEX256_t(ImmutableSparseMatrix_INT32_t_COMPLEX256_t):
    ...
    def copy(self):
        ...
        cdef CSRSparseMatrix_INT32_t_COMPLEX256_t self_copy
        ...
        self_copy = CSRSparseMatrix_INT32_t_COMPLEX256_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, store_zeros=self.__store_zeros, is_symmetric=self.__is_symmetric)

        val = <COMPLEX256_t *> PyMem_Malloc(nnz * sizeof(COMPLEX256_t))
        if not val:
            raise MemoryError()
        memcpy(val, self.val, nnz * sizeof(COMPLEX256_t))
        self_copy.val = val
    ...
    def tril(self, int k):
        ...
        return MakeCSRSparseMatrix_INT32_t_COMPLEX256_t(self.__nrow, self.__ncol, nnz, ind, col, val, is_symmetric=False, store_zeros=self.__store_zeros)
    ...
    def triu(self, int k):
        ...
        if self.__is_symmetric:
            ...
            csc_val = <COMPLEX256_t *> PyMem_Malloc(self.__nnz * sizeof(COMPLEX256_t))
            if not csc_val:
                PyMem_Free(ind)
                PyMem_Free(col)
                PyMem_Free(val)
            ...
            csc_to_csr_kernel_INT32_t_COMPLEX256_t(self.__nrow, self.__ncol, nnz,
    ...
    def to_csc(self):
        ...
        csr_to_csc_kernel_INT32_t_COMPLEX256_t(self.__nrow, self.__ncol, self.__nnz,
                       <INT32_t *>self.ind, <INT32_t *>self.col, <COMPLEX256_t *>self.val,
                       ind, row, val)

        return MakeCSCSparseMatrix_INT32_t_COMPLEX256_t(self.__nrow, self.__ncol, self.__nnz, ind, row, val, is_symmetric=self.is_symmetric, store_zeros=self.store_zeros)
    ...
    def matvec(self, b):
        ...
        Return :math:`A * b`.
        """
        return multiply_csr_mat_with_numpy_vector_INT32_t_COMPLEX256_t(self, b)

    def matvec_transp(self, b):
        """
        Return :math:`A^t * b`.
        """
        return multiply_transposed_csr_mat_with_numpy_vector_INT32_t_COMPLEX256_t(self, b)
    ...
    def matvec_htransp(self, b):
        ...
        return multiply_conjugate_transposed_csr_mat_with_numpy_vector_INT32_t_COMPLEX256_t(self, b)
    ...
    def matvec_conj(self, b):
        ...
        """
        assert are_mixed_types_compatible(COMPLEX256_T, b.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(COMPLEX256_T)
        return multiply_conjugated_csr_mat_with_numpy_vector_INT32_t_COMPLEX256_t(self, b)
    ...
    def __mul__(self, other):
        ...
        # test if implemented
        if isinstance(other, (MutableSparseMatrix_INT32_t_COMPLEX256_t, ImmutableSparseMatrix_INT32_t_COMPLEX256_t)):
            pass
        else:
            raise NotImplemented("Multiplication not (yet) allowed")

        # CASES
        if isinstance(other, CSCSparseMatrix_INT32_t_COMPLEX256_t):
            return multiply_csr_mat_by_csc_mat_INT32_t_COMPLEX256_t(self, other)
        else:
            raise NotImplemented("Multiplication not (yet) allowed")
...
# Factory methods
########################################################################################################################
cdef MakeCSRSparseMatrix_INT32_t_COMPLEX256_t(INT32_t nrow, INT32_t ncol, INT32_t nnz, INT32_t * ind, INT32_t * col, COMPLEX256_t * val, bint is_symmetric, bint store_zeros):
    """
    ...
    csr_mat = CSRSparseMatrix_INT32_t_COMPLEX256_t(control_object=unexposed_value, nrow=nrow, ncol=ncol, nnz=nnz, is_symmetric=is_symmetric, store_zeros=store_zeros)
