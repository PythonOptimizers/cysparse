...
cdef class LLSparseMatrix_INT32_t_INT64_t(MutableSparseMatrix_INT32_t_INT64_t):
    ...
    def copy(self):
        ...
        self_copy = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=self.__use_symmetric_storage)
    ...
    def create_transpose(self):
        ...
        if self.__use_symmetric_storage:
            ...
            transpose = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=self.__use_symmetric_storage)
    ...
    cdef take_triplet_pointers(self, INT32_t * a_row, INT32_t * a_col, INT64_t * a_val):
        """
        Warning:
    ...
    def tril(self, int k):
        ...
        ll_mat_tril = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=False)
    ...
    def triu(self, int k):
        ...
        ll_mat_triu = LLSparseMatrix_INT32_t_INT64_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, use_zero_storage=self.__use_zero_storage, use_symmetric_storage=False)
    ...
    def print_to(self, OUT, width=9, print_big_matrices=False, transposed=False):
        ...
        if print_big_matrices or (self.__nrow <= LL_MAT_PPRINT_COL_THRESH and self.__ncol <= LL_MAT_PPRINT_ROW_THRESH):
            ...
            mat = <INT64_t *> PyMem_Malloc(self.__nrow * self.__ncol * sizeof(INT64_t))
