...
from cysparse.sparse.ll_mat_views.ll_mat_view_INT64_t_FLOAT128_t cimport PyLLSparseMatrixView_Check
...
from cysparse.sparse.csr_mat_matrices.csr_mat_INT64_t_FLOAT128_t cimport MakeCSRSparseMatrix_INT64_t_FLOAT128_t
...
include "ll_mat_kernel/ll_mat_assignment_kernel_INT64_t_FLOAT128_t.pxi"
include "ll_mat_kernel/ll_mat_multiplication_by_numpy_vector_kernel_INT64_t_FLOAT128_t.pxi"
include "ll_mat_helpers/ll_mat_multiplication_INT64_t_FLOAT128_t.pxi"
include "ll_mat_helpers/ll_mat_addition_INT64_t_FLOAT128_t.pxi"
include "ll_mat_helpers/ll_mat_is_symmetric_INT64_t_FLOAT128_t.pxi"
...
########################################################################################################################
from cysparse.sparse.ll_mat_views.ll_mat_view_INT64_t_FLOAT128_t cimport LLSparseMatrixView_INT64_t_FLOAT128_t, MakeLLSparseMatrixView_INT64_t_FLOAT128_t


########################################################################################################################
# CLASS LLSparseMatrix
########################################################################################################################
cdef class LLSparseMatrix_INT64_t_FLOAT128_t(MutableSparseMatrix_INT64_t_FLOAT128_t):
    """
    ...
    def is_symmetric(self):
        ...
        return is_symmetric_INT64_t_FLOAT128_t(self)
    ...
    def copy(self):
        ...
        cdef LLSparseMatrix_INT64_t_FLOAT128_t self_copy

        # we copy manually the C-arrays
        self_copy = LLSparseMatrix_INT64_t_FLOAT128_t(control_object=unexposed_value, no_memory=True, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.size_hint, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def create_transpose(self):
        ...
        cdef:
            INT64_t i, j, k
            LLSparseMatrix_INT64_t_FLOAT128_t transpose

        if self.__store_symmetric:
            return self.copy()
        else:
            transpose = LLSparseMatrix_INT64_t_FLOAT128_t(control_object=unexposed_value, nrow=self.__ncol, ncol=self.__nrow, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=self.__store_symmetric)
    ...
    def to_csr(self):
        ...
        csr_mat = MakeCSRSparseMatrix_INT64_t_FLOAT128_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind, col=col,
                                                     val=val,
    ...
    def to_csc(self):
        ...
        csc_mat = MakeCSCSparseMatrix_INT64_t_FLOAT128_t(nrow=self.__nrow,
                                                     ncol=self.__ncol,
                                                     nnz=self.__nnz,
                                                     ind=ind,
                                                     row=row,
    ...
    ####################################################################################################################
    #                                            ### ASSIGN ###
    cdef assign(self, LLSparseMatrixView_INT64_t_FLOAT128_t view, object obj):
        """
        Set ``A[..., ...] = obj`` directly.

        Args:
            view: An ``LLSparseMatrixView_INT64_t_FLOAT128_t`` that points to this matrix (``self``).
            obj: Can be a:
                - ``LLSparseMatrix``;
                - ``LLSparseMatrixView``;
        ...
        cdef:
            LLSparseMatrix_INT64_t_FLOAT128_t A
            LLSparseMatrixView_INT64_t_FLOAT128_t A_view
        ...
        if self.__store_symmetric:
            if PySparseMatrix_Check(obj):
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A = <LLSparseMatrix_INT64_t_FLOAT128_t> obj
                ...
                if obj.dtype == self.dtype and obj.itype == self.itype:
                    A_view = <LLSparseMatrixView_INT64_t_FLOAT128_t> obj
    ...
    ####################################################################################################################
    #                                            *** SET ***
    cdef put(self, INT64_t i, INT64_t j, FLOAT128_t value):
        """
    ...
    cdef safe_put(self, INT64_t i, INT64_t j, FLOAT128_t value):
        """
    ...
    ####################################################################################################################
    #                                            *** GET ***
    cdef FLOAT128_t at(self, INT64_t i, INT64_t j):
        """
        Return element ``(i, j)``.
    ...
    def __setitem__(self, tuple key, value):
        ...
        cdef LLSparseMatrixView_INT64_t_FLOAT128_t view
        ...
        if not safe_cast_is_integer(key[0]) or not safe_cast_is_integer(key[1]):
            ...
            view = MakeLLSparseMatrixView_INT64_t_FLOAT128_t(self, <PyObject *>key[0], <PyObject *>key[1])
    ...
    cdef fill_triplet(self, INT64_t * a_row, INT64_t * a_col, FLOAT128_t * a_val):
        """
        Warning:
    ...
    def tril(self, int k = 0):
        ...
        cdef:
            INT64_t i, j, k_
            LLSparseMatrix_INT64_t_FLOAT128_t ll_mat_tril

        ll_mat_tril = LLSparseMatrix_INT64_t_FLOAT128_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    def triu(self, int k = 0):
        ...
        cdef:
            INT64_t i, j, k_
            LLSparseMatrix_INT64_t_FLOAT128_t ll_mat_triu

        ll_mat_triu = LLSparseMatrix_INT64_t_FLOAT128_t(control_object=unexposed_value, nrow=self.__nrow, ncol=self.__ncol, size_hint=self.__nnz, store_zero=self.__store_zero, store_symmetric=False)
    ...
    # Addition
    ####################################################################################################################
    def shift(self, sigma, LLSparseMatrix_INT64_t_FLOAT128_t B):
        ...
        if self.__store_symmetric == B.__store_symmetric:
            ...
            for i from 0 <= i < B.__nrow:
                ...
                while k != -1:
                    update_ll_mat_item_add_INT64_t_FLOAT128_t(self, i, B.col[k], casted_sigma * B.val[k])
                    k = B.link[k]
            ...
            for i from 0 <= i < B.__nrow:
                ...
                while k != -1:
                    ...
                    update_ll_mat_item_add_INT64_t_FLOAT128_t(self, i, j, v)
                    if i != j:
                        update_ll_mat_item_add_INT64_t_FLOAT128_t(self, j, i, v)
                    k = B.link[k]
        else:
    ...
    def update_add_at(self, id1, id2, val):
        ...
        See :meth:`update_add_at_with_numpy_arrays_INT64_t_FLOAT128_t`.
        """
        return update_add_at_with_numpy_arrays_INT64_t_FLOAT128_t(self, id1, id2, val)
    ...
    def matdot(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            return multiply_two_ll_mat_INT64_t_FLOAT128_t(self, B)
    ...
    def matdot_transp(self, B):
        ...
        if PyLLSparseMatrix_Check(B):
            ...
            return multiply_transposed_ll_mat_by_ll_mat_INT64_t_FLOAT128_t(self, B)
        elif cnp.PyArray_Check(B):
            assert are_mixed_types_compatible(FLOAT128_T, B.dtype), "Multiplication only allowed with a Numpy compatible type (%s)!" % cysparse_to_numpy_type(FLOAT128_T)
...
# Factory methods
########################################################################################################################
cdef MakeLLSparseMatrix_INT64_t_FLOAT128_t(INT64_t nrow,
                                        INT64_t ncol,
                                        INT64_t nnz,
                                        INT64_t free,
                                        INT64_t nalloc,
    ...
    """
    cdef LLSparseMatrix_INT64_t_FLOAT128_t ll_mat

    ll_mat = LLSparseMatrix_INT64_t_FLOAT128_t(control_object=unexposed_value,
                                           no_memory=True,
                                           size_hint=nalloc,
                                           nrow=nrow,
                                           ncol=ncol,
