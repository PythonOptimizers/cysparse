...
##################################
class CySparseCommonAttributesMatrices_ConjugateTransposedSparseMatrix_INT64_t_COMPLEX256_t_TestCase(unittest.TestCase):
    def setUp(self):
        self.nrow = 10
        self.ncol = 14
        self.nnz = self.nrow * self.ncol
        self.A = LinearFillLLSparseMatrix(nrow=self.nrow, ncol=self.ncol, dtype=COMPLEX256_T, itype=INT64_T)
...
##################################
class CySparseCommonAttributesSymmetricMatrices_ConjugateTransposedSparseMatrix_INT64_t_COMPLEX256_t_TestCase(unittest.TestCase):
    def setUp(self):
        self.size = 14
        self.nnz = ((self.size + 1) * self.size) / 2

        self.A = LinearFillLLSparseMatrix(size=self.size, dtype=COMPLEX256_T, itype=INT64_T, store_symmetric=True)
...
##################################
class CySparseCommonAttributesWithZeroMatrices_ConjugateTransposedSparseMatrix_INT64_t_COMPLEX256_t_TestCase(unittest.TestCase):
    def setUp(self):
        self.nrow = 10
        self.ncol = 14
        self.nnz = self.nrow * self.ncol
        self.A = LinearFillLLSparseMatrix(nrow=self.nrow, ncol=self.ncol, dtype=COMPLEX256_T, itype=INT64_T, store_zero=True)
...
# Case Symmetric, Zero
##################################
class CySparseCommonAttributesSymmetricWithZeroMatrices_ConjugateTransposedSparseMatrix_INT64_t_COMPLEX256_t_TestCase(unittest.TestCase):
    def setUp(self):
        self.size = 14
        self.nnz = ((self.size + 1) * self.size) / 2
        self.A = LinearFillLLSparseMatrix(size=self.size, dtype=COMPLEX256_T, itype=INT64_T, store_symmetric=True, store_zero=True)
